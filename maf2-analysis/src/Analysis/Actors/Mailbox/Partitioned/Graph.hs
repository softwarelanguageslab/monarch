...
-- | Copy of Analysis.Actors.Mailbox.Graph but with 
-- partitioned top, bottom pointers and node values.
module Analysis.Actors.Mailbox.Partitioned.Graph (
    -- * Graph abstraction
    PartitionedGraph(..),
    Node(..),
    PartitionedNode,
...
import Analysis.Actors.Mailbox.Partitioned (PartitionedMailbox(..), IsPartition, isConcurrent)
...
-- | A non-partitioned node which includes a regular node and a terminal node,
...
instance (IsPartition p, Ord p, Ord a) => PartitionedMailbox p a (PartitionedGraph p a) where
  -- Recall properties:
...
-- toGraph :: (Ord a) => PartitionedGraph p a -> RegularGraph.Graph a
