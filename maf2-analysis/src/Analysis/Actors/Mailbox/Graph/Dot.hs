...
module Analysis.Actors.Mailbox.Graph.Dot(render) where
...
import qualified Analysis.Actors.Mailbox.Partitioned.Graph as PartitionedGraph
import Analysis.Actors.Mailbox.Graph (Node)
import Data.Map (Map)
...
colorNode :: (MonadRender c p node m) => c -> m String
colorNode contents =
    nodeColor <$>
...
colorPartition :: (MonadRender c p node m) => p -> m String
...
-- in the edge list.
renderNodes :: (MonadRender c p node m, Show node) => [node] -> m String
renderNodes =
    foldM (\acc node -> do
...
renderEdges :: (MonadRender c p node m, Show node) => [(node, node)] -> m String
renderEdges =
...
render :: (Ord a, Show a, Ord p, Show p) => PartitionedGraph p a -> String
render graph =
    let initialState = State Map.empty Map.empty Map.empty 0 0
        renderGraph = do
            ...
            return $ "digraph Mailbox {\n" <> nodeString <> edgeString <> pointerString <> "}\n"
