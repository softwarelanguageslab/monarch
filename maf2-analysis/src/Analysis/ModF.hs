...
-- import Domain
-- import Domain.Scheme hiding (Env, Exp)
...
-- test 
eval :: Exp -> Maybe ()
eval e = undefined {- fst $ runIntra e sto () env (\_ -> const $ Ctx ())
   where env = initialEnv 
         sto = initialSto env -}

evalStr :: String -> Maybe ()
evalStr = undefined -- Analysis.ModF.eval . fromJust . parseSchemeExp
