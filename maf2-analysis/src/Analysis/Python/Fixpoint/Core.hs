...
module Analysis.Python.Fixpoint.Core where
...
analyze :: forall obj . (Typeable obj, Show obj, PyDomain obj PyRef) => PyPrg -> (Map PyCmp PyRes, Store obj)
analyze prg = (rsto, osto)
    where ((osto, _), rsto) = inter @obj prg
...
analyzeCP :: PyPrg -> (Map PyCmp PyRes, Store PyDomainCP)
