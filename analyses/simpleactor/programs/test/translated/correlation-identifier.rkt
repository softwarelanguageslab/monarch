      ...
      (letrec ((empty '())
                  ...
                  (lambda (v2650)
                    (letrec ((result2667
                              ((lambda (k2652 j2653 v2654)
                                 (match
                                  v2654
                                  (((cons 'request (cons x2655 x2656))
                                    (letrec ((x2657 (actor? k2652 j2653 x2655))
                                             (x2658
                                              (number?/c k2652 j2653 x2656)))
                                      (cons
                                       'enhanced
                                       (cons
                               ...
                               j2649
                               v2650)))
                      (if result2667 (a2647 result2667) (blame k2648))))))
               (server-behavior
                (lambda ()
                     ...
                     (receive
                      (((cons 'request (cons ref identifier))
                       ...
                       ((cons
                         ...
                         (cons
                          k2669
                          (cons j2673 (cons 'request (cons ref identifier)))))
                        (letrec ((kc2670 (k2669 j2673))
                                 (old-send2674 (dyn send^)))
                          (parametrize
                           ((send^
                     ...
                     (receive
                      (((cons 'reply identifier)
                       ...
                       ((cons
                        ...
                        (letrec ((kc2677 (k2676 j2680))
                                 (old-send2681 (dyn send^)))
                          (parametrize
                           ((send^
                             (lambda (rcv2678 msg2679)
               ...
               (client
                (letrec ((act (spawn^ (client-behavior))))
                  (lambda (msg) ((dyn send^) act msg))))
               (server
                (letrec ((xj2683 (loc 'client)) (xk2684 (loc 'server)))
                  (server/c xj2683 xk2684 unmonitored-server))))
