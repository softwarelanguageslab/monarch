      ...
      (letrec ((empty '())
                    ...
                    (parametrize
                     ...
                     (receive
                      (((cons 'request sender)
                       ...
                       ((cons
                         'enhanced
                         (cons k8976 (cons j8980 (cons 'request sender))))
                        (letrec ((kc8977 (k8976 j8980))
                                 (old-send8981 (dyn send^)))
                          (parametrize
                           ((send^
                             (lambda (rcv8978 msg8979)
                               (old-send8981 kc8977 (cons rcv8978 msg8979)))))
                           (begin
                             (sender 'reply)
                             (old-send8981 kc8977 'finish)
                             (server-behavior)))))))))))
               (client-behavior
                (lambda ()
                  (letrec ((real-self (self^)))
                    (parametrize
                     ...
                     (receive
                      (((cons 'start server)
                       ...
                       ((cons
                         'enhanced
                         (cons k8982 (cons j8986 (cons 'start server))))
                        (letrec ((kc8983 (k8982 j8986))
                                 (old-send8987 (dyn send^)))
                          (parametrize
                           ((send^
                             (lambda (rcv8984 msg8985)
                               (old-send8987 kc8983 (cons rcv8984 msg8985)))))
                           (begin
                             (server (cons 'request (dyn self)))
                             (old-send8987 kc8983 'finish)
                             (client-behavior)))))
                       ((cons 'enhanced (cons k8988 (cons j8992 'reply)))
                        (letrec ((kc8989 (k8988 j8992))
                                 (old-send8993 (dyn send^)))
                          (parametrize
                           ((send^
                             (lambda (rcv8990 msg8991)
                               (old-send8993 kc8989 (cons rcv8990 msg8991)))))
               ...
               (server-unmonitored
                (letrec ((act (spawn^ (server-behavior))))
                  (lambda (msg) ((dyn send^) act msg))))
               (server
                (letrec ((xj8994 (loc 'client)) (xk8995 (loc 'server)))
                  (server/c xj8994 xk8995 server-unmonitored))))
