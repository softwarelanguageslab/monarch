    ...
    (letrec ((assert (lambda (cnd) (if cnd '() (error "assertion failed"))))
      ...
      (letrec ((empty '())
        ...
        (letrec ((N 10)
                 (A 3)
                 (perform-computation
                  (lambda (theta) (letrec ((sint (+ 1 theta))) (* sint sint))))
                 (throughput-actor
                  (lambda (processed)
                       ...
                       (receive
                        (('message
                          (begin
                            ...
                            (if (= (+ processed 1) N)
                              (terminate)
                              (throughput-actor (+ processed 1)))))
                         ((cons 'enhanced (cons k7681 (cons j7685 'message)))
                 ...
                 (actors
                  (build-vector
                   A
                   (lambda (i)
                     (letrec ((act (spawn^ (throughput-actor 0))))
                 ...
                 (loop
                  (lambda (n)
                    (if (= n N)
                      'done
                      (begin
                        (vector-foreach (lambda (a) (a 'message)) actors)
                        (loop (+ n 1)))))))
          (loop 0))))))
