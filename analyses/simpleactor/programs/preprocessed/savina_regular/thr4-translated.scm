        ...
        (letrec ((N 3)
                       ...
                       (receive
                        (((cons 'ping pings-left)
                          (begin
                            (if (> pings-left 0)
                              (begin
                                (next-actor (cons 'ping (- pings-left 1)))
                                (threadring-actor
                                 id
                                 actors-in-ring
                                 next-actor))
                              (begin
                                (next-actor (cons 'exit actors-in-ring))
                                (threadring-actor
                                 id
                                 actors-in-ring
                                 next-actor)))))
                         ...
                         ((cons 'exit exits-left)
                          (begin (if (> exits-left 1) '() #f) (terminate)))
                         ((cons
                           'enhanced
                 ...
                 (ring-actors
                  (build-vector
                   N
                   (lambda (i)
                     (letrec ((act (spawn^ (threadring-actor i N #f))))
                 ...
                 (loop-next
                  (lambda (i)
                    (if (= i N)
                      'done
                      (begin
                        ((vector-ref ring-actors i)
                         (cons
                          'data
                          (vector-ref ring-actors (modulo (+ i 1) N))))
                        (loop-next (+ i 1)))))))
          (loop-next 0)
          ((vector-ref ring-actors 0) (cons 'ping R)))))))
