                 ...
                 (vector-foreach
                  (lambda (f v)
                    (letrec ((loop
                      ...
                      (loop 0))))
                 (big-actor
                  (lambda (id num-pings sink exp-pinger neighbors)
                    (letrec ((real-self (self^)))
                      (parametrize
                       ...
                       (receive
                        (((cons 'ping sender-id)
                          (begin
                            ((vector-ref neighbors sender-id) (cons 'pong id))
                            (big-actor
                             id
                             num-pings
                             sink
                            ...
                            (if (= sender exp-pinger)
                              (if (= num-pings num-messages)
                                (begin
                                  ...
                                  (big-actor
                                   ...
                                   neighbors))
                                (letrec ((target-id
                                          (random (vector-length neighbors)))
                                         (target
                                          (vector-ref neighbors target-id)))
                                  (target (cons 'ping id))
                                  (big-actor
                                   id
                                   (+ num-pings 1)
                              ...
                              (error
                               (format
                                ...
                                exp-pinger
                                sender)))))
                         ('exit (begin (terminate)))
                         ((cons 'neighbors new-neighbors)
                          (begin
                            (big-actor
                             id
                             num-pings
                         ...
                         ((cons
                           'enhanced
                           (cons k7681 (cons j7685 (cons 'ping sender-id))))
                          (letrec ((kc7682 (k7681 j7685))
                            ...
                            (parametrize
                             ...
                             (begin
                               ((vector-ref neighbors sender-id)
                                (cons 'pong id))
                               (old-send7686 kc7682 'finish)))
                            (big-actor
                             id
                             num-pings
                             sink
                         ...
                         ((cons
                           'enhanced
                           (cons k7688 (cons j7692 (cons 'pong sender))))
                            ...
                            (if (= sender exp-pinger)
                              ...
                              (error
                               (format
                                ...
                                exp-pinger
                                sender)))))
                         ((cons 'enhanced (cons k7695 (cons j7699 'exit)))
                         ...
                         ((cons
                           ...
                           (cons
                            k7702
                            (cons j7706 (cons 'neighbors new-neighbors))))
                          (letrec ((kc7703 (k7702 j7706))
                            ...
                            (parametrize
                             ...
                             (begin (old-send7707 kc7703 'finish)))
                            (big-actor
                             id
                             num-pings
                             sink
                        ...
                        (('exit
                          (begin
                            (if (= (+ num-messages 1) num-workers)
                              (begin
                                (vector-foreach (lambda (a) '()) neighbors)
                                (terminate))
                              (sink-actor (+ num-messages 1) neighbors))))
                         ((cons 'neighbors new-neighbors)
                          (begin (sink-actor num-messages new-neighbors)))
                         ((cons 'enhanced (cons k7709 (cons j7713 'exit)))
                          (letrec ((kc7710 (k7709 j7713))
                            ...
                            (parametrize
                             ...
                             (begin (old-send7714 kc7710 'finish)))
                            (if (= (+ num-messages 1) num-workers)
                              (begin
                                (vector-foreach (lambda (a) '()) neighbors)
                                (terminate))
                         ...
                         ((cons
                           ...
                           (cons
                            k7716
                            (cons j7720 (cons 'neighbors new-neighbors))))
                          (letrec ((kc7717 (k7716 j7720))
                                   (old-send7721 (dyn send^)))
                            (parametrize
                             ((send^
                               (lambda (rcv7718 msg7719)
                                 (old-send7721
                                  kc7717
                                  (cons rcv7718 msg7719)))))
                             (begin (old-send7721 kc7717 'finish)))
                            (sink-actor num-messages new-neighbors)))))))))
                 (sink
                  (letrec ((act (spawn^ (sink-actor 0 #f))))
                    (lambda (msg) ((dyn send^) act msg))))
                 (big-actors
                  (vector
                   (letrec ((act (spawn^ (big-actor 0 0 sink -1 #f))))
                     (lambda (msg) ((dyn send^) act msg)))
                   (letrec ((act (spawn^ (big-actor 1 0 sink -1 #f))))
                     (lambda (msg) ((dyn send^) act msg))))))
          (sink (cons 'neighbors big-actors))
          (vector-foreach
           (lambda (a) (a (cons 'neighbors big-actors)))
           big-actors)
          (vector-foreach (lambda (a) (a (cons 'pong -1))) big-actors))))))
