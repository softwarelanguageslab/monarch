                      ...
                      (parametrize
                       ...
                       (receive
                        (('increment
                            ...
                            (letrec ((loop
                                      (lambda (n)
                                        (if (> n 0)
                                          ...
                                          'done))))
                              (loop N)
                              (counter (cons 'retrieve (dyn self)))
                              (producer-actor counter))))
                         ((cons 'result count)
                          (begin
                            (if (= count N)
                              (display "Success!")
                              (error "Error!"))
                            (terminate)))
                         ((cons 'enhanced (cons k7681 (cons j7685 'increment)))
                         ...
                         ((cons
                           'enhanced
                           (cons k7688 (cons j7692 (cons 'result count))))
                  ...
                  (lambda (count)
                    (letrec ((real-self (self^)))
                      (parametrize
                       ((self (lambda (m) ((dyn send^) real-self m))))
                       (receive
                        (('increment (begin (counting-actor (+ count 1))))
                         ((cons 'retrieve to) (begin '() (terminate)))
                         ((cons 'enhanced (cons k7695 (cons j7699 'increment)))
                          (letrec ((kc7696 (k7695 j7699))
                            ...
                            (counting-actor (+ count 1))))
                         ((cons
                           'enhanced
                           (cons k7702 (cons j7706 (cons 'retrieve to))))
                          (letrec ((kc7703 (k7702 j7706))
                            ...
                            (terminate)))))))))
                 (counter
                  (letrec ((act (spawn^ (counting-actor 0))))
                    (lambda (msg) ((dyn send^) act msg))))
                 (producer
                  (letrec ((act (spawn^ (producer-actor counter))))
                    (lambda (msg) ((dyn send^) act msg)))))
          (producer 'increment))))))
