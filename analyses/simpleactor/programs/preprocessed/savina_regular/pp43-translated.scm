                  ...
                  (lambda (count pong)
                    (letrec ((real-self (self^)))
                      (parametrize
                       ...
                       (receive
                        (('start (begin '() (ping-actor (- count 1) pong)))
                         ('ping
                          (begin
                            (pong (cons 'send-ping (dyn self)))
                            (ping-actor (- count 1) pong)))
                         ('send-pong
                          (begin
                            (if (> count 0)
                              (begin '() (ping-actor count pong))
                              (begin (pong 'stop) (terminate)))))
                         ((cons 'enhanced (cons k7681 (cons j7685 'start)))
                          (letrec ((kc7682 (k7681 j7685))
                            ...
                            (ping-actor (- count 1) pong)))
                         ((cons 'enhanced (cons k7688 (cons j7692 'ping)))
                          (letrec ((kc7689 (k7688 j7692))
                            ...
                            (parametrize
                             ...
                             (begin
                               (pong (cons 'send-ping (dyn self)))
                               (old-send7693 kc7689 'finish)))
                            (ping-actor (- count 1) pong)))
                         ((cons 'enhanced (cons k7695 (cons j7699 'send-pong)))
                  ...
                  (lambda (count)
                    (letrec ((real-self (self^)))
                      (parametrize
                       ((self (lambda (m) ((dyn send^) real-self m))))
                       (receive
                        (('stop (begin (terminate)))
                         ((cons 'send-ping to)
                          (begin '() (pong-actor (+ count 1))))
                         ((cons 'enhanced (cons k7702 (cons j7706 'stop)))
                         ...
                         ((cons
                           'enhanced
                           (cons k7709 (cons j7713 (cons 'send-ping to))))
                          (letrec ((kc7710 (k7709 j7713))
                            ...
                            (pong-actor (+ count 1))))))))))
                 (pong
                  (letrec ((act (spawn^ (pong-actor 0))))
                    (lambda (msg) ((dyn send^) act msg))))
                 (N 5)
                 (ping
                  (letrec ((act (spawn^ (ping-actor N pong))))
                    (lambda (msg) ((dyn send^) act msg)))))
          '())))))
