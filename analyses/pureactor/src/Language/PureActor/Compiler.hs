...
compileHandlers = fmap (Handlers . Map.fromList) . SExp.smapM compileHandler
  where
    ...
    compileHandler expr = throwError $ "invalid handler at" ++ toString (S.show $ spanOf expr)

-- | Compile a list of S-expressions into a list of expressions
compileSequence :: MonadCompile m => SExp.SExp -> m [Expr]
compileSequence = SExp.smapM compile

-- | Compile a list of bindings
compileBindings :: MonadCompile m => SExp.SExp -> m [(Ide, Expr)]
compileBindings = SExp.smapM compileBinding
  where
    compileBinding (SExp.Atom nam s ::: binding ::: SExp.SNil _) =
      (Ide nam s,) <$> compile binding
    compileBinding expr = throwError $ "invalid letrec binding at" ++ toString (S.show $ spanOf expr)
...
compile expr =
    case expr of
      ...
      (SExp.Atom "letrec" _) ::: bindings ::: body ::: SExp.SNil _ ->
        Letrec <$> compileBindings bindings <*> compile body <*> pure (spanOf expr)
