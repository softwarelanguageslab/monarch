(begin
  (use-modules (srfi srfi-28))
  (define (add1 x) (begin (write '(funapp 3 20)) (display "\n") (+ x 1)))
  (define (sub1 x) (begin (write '(funapp 4 20)) (display "\n") (+ x 1)))
  (define (loc x) x)
  (set! *random-state*
    (begin (write '(funapp 6 24)) (display "\n") (random-state-from-platform)))
  (define (input) (begin (write '(funapp 7 19)) (display "\n") (expt 2 64)))
  (letrec*
   ((any? (lambda (v) (letrec* ((g7631 #t)) g7631)))
    (meta (lambda (v) (letrec* ((g7632 v)) g7632)))
    (member
     (lambda (v lst)
       (letrec*
        ((g7633
          (letrec*
           ((g7634
             (letrec*
              ((x-e7635 lst))
              (letrec*
               ((v1804 x-e7635))
               (cond
                ((begin (write '(funapp 22 18)) (display "\n") (null? v1804))
                 (letrec* () #f))
                ((begin (write '(funapp 23 18)) (display "\n") (and #t #t))
                 (letrec*
                  ((v1
                    (begin (write '(funapp 25 24)) (display "\n") (car v1804)))
                   (vs
                    (begin
                      (write '(funapp 25 41))
                      (display "\n")
                      (cdr v1804))))
                  (letrec*
                   ((x-cnd7636
                     (begin
                       (write '(funapp 27 32))
                       (display "\n")
                       (eq? v v1))))
                   (if x-cnd7636
                     #t
                     (begin
                       (write '(funapp 28 37))
                       (display "\n")
                       (member v vs))))))
                (else
                 (begin
                   (write '(funapp 29 23))
                   (display "\n")
                   (error "no match found"))))))))
           g7634)))
        g7633)))
    (unconstrained/c #f)
    (actor?
     (lambda (k j)
       (letrec* ((g7637 (lambda (v) (letrec* ((g7638 v)) g7638)))) g7637)))
    (nonzero?
     (lambda (v)
       (letrec*
        ((g7639
          (letrec*
           ((x7640 (begin (write '(funapp 38 42)) (display "\n") (= v 0))))
           (begin (write '(funapp 38 52)) (display "\n") (not x7640)))))
        g7639))))
   (letrec*
    ((g7641
      (letrec*
       ((g7642
         (letrec*
          ((orig-+ +)
           (orig-- -)
           (orig-* *)
           (orig-/ /)
           (orig-car car)
           (orig-cdr cdr)
           (orig-cons cons)
           (orig-vector-ref vector-ref)
           (orig-vector-set vector-set!))
          (letrec*
           ((g7643 (begin (write '(funapp 54 19)) (display "\n") '()))
            (g7644
             (letrec*
              ((empty (begin (write '(funapp 57 22)) (display "\n") '()))
               (listof
                (lambda (contract)
                  (letrec*
                   ((g7645
                     (lambda (k j lst)
                       (letrec*
                        ((g7646
                          (begin
                            (write '(funapp 65 27))
                            (display "\n")
                            (map
                             (lambda (v)
                               (letrec*
                                ((g7647
                                  (begin
                                    (write '(funapp 67 47))
                                    (display "\n")
                                    (contract k j v))))
                                g7647))
                             lst))))
                        g7646))))
                   g7645)))
               (real?/c
                (lambda (g7541 g7542 g7543)
                  (letrec*
                   ((g7648
                     (letrec*
                      ((x-cnd7649
                        (begin
                          (write '(funapp 76 35))
                          (display "\n")
                          (real? g7543))))
                      (if x-cnd7649
                        g7543
                        (begin
                          (write '(blame g7541 77 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7541)))))))
                   g7648)))
               (boolean?/c
                (lambda (g7544 g7545 g7546)
                  (letrec*
                   ((g7650
                     (letrec*
                      ((x-cnd7651
                        (begin
                          (write '(funapp 84 35))
                          (display "\n")
                          (boolean? g7546))))
                      (if x-cnd7651
                        g7546
                        (begin
                          (write '(blame g7544 85 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7544)))))))
                   g7650)))
               (number?/c
                (lambda (g7547 g7548 g7549)
                  (letrec*
                   ((g7652
                     (letrec*
                      ((x-cnd7653
                        (begin
                          (write '(funapp 92 35))
                          (display "\n")
                          (number? g7549))))
                      (if x-cnd7653
                        g7549
                        (begin
                          (write '(blame g7547 93 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7547)))))))
                   g7652)))
               (any/c
                (lambda (g7550 g7551 g7552)
                  (letrec*
                   ((g7654
                     (letrec*
                      ((x-cnd7655
                        (begin
                          (write '(funapp 101 25))
                          (display "\n")
                          ((lambda (v) (letrec* ((g7656 #t)) g7656)) g7552))))
                      (if x-cnd7655
                        g7552
                        (begin
                          (write '(blame g7550 102 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7550)))))))
                   g7654)))
               (any?/c
                (lambda (g7553 g7554 g7555)
                  (letrec*
                   ((g7657
                     (letrec*
                      ((x-cnd7658
                        (begin
                          (write '(funapp 110 25))
                          (display "\n")
                          ((lambda (v) (letrec* ((g7659 #t)) g7659)) g7555))))
                      (if x-cnd7658
                        g7555
                        (begin
                          (write '(blame g7553 111 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7553)))))))
                   g7657)))
               (cons?/c
                (lambda (g7556 g7557 g7558)
                  (letrec*
                   ((g7660
                     (letrec*
                      ((x-cnd7661
                        (begin
                          (write '(funapp 118 35))
                          (display "\n")
                          (pair? g7558))))
                      (if x-cnd7661
                        g7558
                        (begin
                          (write '(blame g7556 119 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7556)))))))
                   g7660)))
               (cons/c cons?/c)
               (pair?/c
                (lambda (g7559 g7560 g7561)
                  (letrec*
                   ((g7662
                     (letrec*
                      ((x-cnd7663
                        (begin
                          (write '(funapp 127 35))
                          (display "\n")
                          (pair? g7561))))
                      (if x-cnd7663
                        g7561
                        (begin
                          (write '(blame g7559 128 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7559)))))))
                   g7662)))
               (integer?/c
                (lambda (g7562 g7563 g7564)
                  (letrec*
                   ((g7664
                     (letrec*
                      ((x-cnd7665
                        (begin
                          (write '(funapp 135 35))
                          (display "\n")
                          (integer? g7564))))
                      (if x-cnd7665
                        g7564
                        (begin
                          (write '(blame g7562 136 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7562)))))))
                   g7664)))
               (symbol?/c
                (lambda (g7565 g7566 g7567)
                  (letrec*
                   ((g7666
                     (letrec*
                      ((x-cnd7667
                        (begin
                          (write '(funapp 143 35))
                          (display "\n")
                          (symbol? g7567))))
                      (if x-cnd7667
                        g7567
                        (begin
                          (write '(blame g7565 144 42))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7565)))))))
                   g7666)))
               (and/c
                (lambda (c1 c2)
                  (letrec*
                   ((g7668
                     (lambda (k j v)
                       (letrec*
                        ((g7669
                          (letrec*
                           ((x-cnd7670
                             (begin
                               (write '(funapp 154 40))
                               (display "\n")
                               (c1 k j v))))
                           (if x-cnd7670
                             (begin
                               (write '(funapp 155 42))
                               (display "\n")
                               (c2 k j v))
                             #f))))
                        g7669))))
                   g7668)))
               (list-of
                (lambda (contract)
                  (letrec*
                   ((g7671
                     (lambda (k j v)
                       (letrec*
                        ((g7672
                          (letrec*
                           ((x-cnd7673
                             (begin
                               (write '(funapp 166 40))
                               (display "\n")
                               (null? v))))
                           (if x-cnd7673
                             (begin
                               (write '(funapp 168 29))
                               (display "\n")
                               '())
                             (letrec*
                              ((x7677
                                (letrec*
                                 ((x7678
                                   (begin
                                     (write '(funapp 172 42))
                                     (display "\n")
                                     (car v))))
                                 (begin
                                   (write '(funapp 173 34))
                                   (display "\n")
                                   (contract k j x7678))))
                               (x7674
                                (letrec*
                                 ((x7676
                                   (begin
                                     (write '(funapp 176 42))
                                     (display "\n")
                                     (list-of contract)))
                                  (x7675
                                   (begin
                                     (write '(funapp 176 69))
                                     (display "\n")
                                     (cdr v))))
                                 (begin
                                   (write '(funapp 177 34))
                                   (display "\n")
                                   (x7676 k j x7675)))))
                              (begin
                                (write '(funapp 178 31))
                                (display "\n")
                                (orig-cons x7677 x7674)))))))
                        g7672))))
                   g7671)))
               (any? (lambda (v) (letrec* ((g7679 #t)) g7679)))
               (nonzero?
                (lambda (v)
                  (letrec*
                   ((g7680
                     (letrec*
                      ((x7681
                        (begin
                          (write '(funapp 185 45))
                          (display "\n")
                          (= v 0))))
                      (begin
                        (write '(funapp 185 55))
                        (display "\n")
                        (not x7681)))))
                   g7680)))
               (nonzero?/c
                (lambda (g7568 g7569 g7570)
                  (letrec*
                   ((g7682
                     (letrec*
                      ((x-cnd7683
                        (begin
                          (write '(funapp 193 25))
                          (display "\n")
                          ((lambda (v)
                             (letrec*
                              ((g7684
                                (letrec*
                                 ((x7685
                                   (begin
                                     (write '(funapp 195 54))
                                     (display "\n")
                                     (= v 0))))
                                 (begin
                                   (write '(funapp 195 64))
                                   (display "\n")
                                   (not x7685)))))
                              g7684))
                           g7570))))
                      (if x-cnd7683
                        g7570
                        (begin
                          (write '(blame g7568 200 24))
                          (display "\n")
                          (error
                           (format
                            "contract violation, blaming ~a~%"
                            g7568)))))))
                   g7682)))
               (meta (lambda (v) (letrec* ((g7686 v)) g7686)))
               (+
                (letrec*
                 ((xj7571
                   (begin (write '(funapp 205 26)) (display "\n") 'server))
                  (xk7572
                   (begin (write '(funapp 205 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7687
                    (begin
                      (write '(funapp 208 21))
                      (display "\n")
                      ((lambda (j7575 k7576 f7577)
                         (letrec*
                          ((g7689
                            (lambda (g7573 g7574)
                              (letrec*
                               ((g7690
                                 (letrec*
                                  ((x7691
                                    (letrec*
                                     ((x7693
                                       (begin
                                         (write '(funapp 217 44))
                                         (display "\n")
                                         (number?/c j7575 k7576 g7573)))
                                      (x7692
                                       (begin
                                         (write '(funapp 218 44))
                                         (display "\n")
                                         (number?/c j7575 k7576 g7574))))
                                     (begin
                                       (write '(funapp 219 36))
                                       (display "\n")
                                       (f7577 x7693 x7692)))))
                                  (begin
                                    (write '(funapp 220 33))
                                    (display "\n")
                                    (number?/c j7575 k7576 x7691)))))
                               g7690))))
                          g7689))
                       xj7571
                       xk7572
                       (lambda (a b)
                         (letrec*
                          ((g7688
                            (begin
                              (write '(funapp 225 53))
                              (display "\n")
                              (orig-+ a b))))
                          g7688))))))
                  g7687)))
               (-
                (letrec*
                 ((xj7578
                   (begin (write '(funapp 229 26)) (display "\n") 'server))
                  (xk7579
                   (begin (write '(funapp 229 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7694
                    (begin
                      (write '(funapp 232 21))
                      (display "\n")
                      ((lambda (j7582 k7583 f7584)
                         (letrec*
                          ((g7696
                            (lambda (g7580 g7581)
                              (letrec*
                               ((g7697
                                 (letrec*
                                  ((x7698
                                    (letrec*
                                     ((x7700
                                       (begin
                                         (write '(funapp 241 44))
                                         (display "\n")
                                         (number?/c j7582 k7583 g7580)))
                                      (x7699
                                       (begin
                                         (write '(funapp 242 44))
                                         (display "\n")
                                         (number?/c j7582 k7583 g7581))))
                                     (begin
                                       (write '(funapp 243 36))
                                       (display "\n")
                                       (f7584 x7700 x7699)))))
                                  (begin
                                    (write '(funapp 244 33))
                                    (display "\n")
                                    (number?/c j7582 k7583 x7698)))))
                               g7697))))
                          g7696))
                       xj7578
                       xk7579
                       (lambda (a b)
                         (letrec*
                          ((g7695
                            (begin
                              (write '(funapp 249 53))
                              (display "\n")
                              (orig-- a b))))
                          g7695))))))
                  g7694)))
               (*
                (letrec*
                 ((xj7585
                   (begin (write '(funapp 253 26)) (display "\n") 'server))
                  (xk7586
                   (begin (write '(funapp 253 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7701
                    (begin
                      (write '(funapp 256 21))
                      (display "\n")
                      ((lambda (j7589 k7590 f7591)
                         (letrec*
                          ((g7703
                            (lambda (g7587 g7588)
                              (letrec*
                               ((g7704
                                 (letrec*
                                  ((x7705
                                    (letrec*
                                     ((x7707
                                       (begin
                                         (write '(funapp 265 44))
                                         (display "\n")
                                         (number?/c j7589 k7590 g7587)))
                                      (x7706
                                       (begin
                                         (write '(funapp 266 44))
                                         (display "\n")
                                         (number?/c j7589 k7590 g7588))))
                                     (begin
                                       (write '(funapp 267 36))
                                       (display "\n")
                                       (f7591 x7707 x7706)))))
                                  (begin
                                    (write '(funapp 268 33))
                                    (display "\n")
                                    (number?/c j7589 k7590 x7705)))))
                               g7704))))
                          g7703))
                       xj7585
                       xk7586
                       (lambda (a b)
                         (letrec*
                          ((g7702
                            (begin
                              (write '(funapp 273 53))
                              (display "\n")
                              (orig-* a b))))
                          g7702))))))
                  g7701)))
               (/
                (letrec*
                 ((xj7592
                   (begin (write '(funapp 277 26)) (display "\n") 'server))
                  (xk7593
                   (begin (write '(funapp 277 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7708
                    (begin
                      (write '(funapp 280 21))
                      (display "\n")
                      ((lambda (j7596 k7597 f7598)
                         (letrec*
                          ((g7710
                            (lambda (g7594 g7595)
                              (letrec*
                               ((g7711
                                 (letrec*
                                  ((x7712
                                    (letrec*
                                     ((x7714
                                       (begin
                                         (write '(funapp 289 44))
                                         (display "\n")
                                         (number?/c j7596 k7597 g7594)))
                                      (x7713
                                       (begin
                                         (write '(funapp 290 44))
                                         (display "\n")
                                         (number?/c j7596 k7597 g7595))))
                                     (begin
                                       (write '(funapp 291 36))
                                       (display "\n")
                                       (f7598 x7714 x7713)))))
                                  (begin
                                    (write '(funapp 292 33))
                                    (display "\n")
                                    (number?/c j7596 k7597 x7712)))))
                               g7711))))
                          g7710))
                       xj7592
                       xk7593
                       (lambda (a b)
                         (letrec*
                          ((g7709
                            (begin
                              (write '(funapp 297 53))
                              (display "\n")
                              (orig-/ a b))))
                          g7709))))))
                  g7708)))
               (car
                (letrec*
                 ((xj7599
                   (begin (write '(funapp 301 26)) (display "\n") 'server))
                  (xk7600
                   (begin (write '(funapp 301 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7715
                    (begin
                      (write '(funapp 304 21))
                      (display "\n")
                      ((lambda (j7602 k7603 f7604)
                         (letrec*
                          ((g7717
                            (lambda (g7601)
                              (letrec*
                               ((g7718
                                 (letrec*
                                  ((x7719
                                    (letrec*
                                     ((x7720
                                       (begin
                                         (write '(funapp 313 44))
                                         (display "\n")
                                         (pair?/c j7602 k7603 g7601))))
                                     (begin
                                       (write '(funapp 314 36))
                                       (display "\n")
                                       (f7604 x7720)))))
                                  (begin
                                    (write '(funapp 315 33))
                                    (display "\n")
                                    (any/c j7602 k7603 x7719)))))
                               g7718))))
                          g7717))
                       xj7599
                       xk7600
                       (lambda (p)
                         (letrec*
                          ((g7716
                            (begin
                              (write '(funapp 320 51))
                              (display "\n")
                              (orig-car p))))
                          g7716))))))
                  g7715)))
               (cdr
                (letrec*
                 ((xj7605
                   (begin (write '(funapp 324 26)) (display "\n") 'server))
                  (xk7606
                   (begin (write '(funapp 324 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7721
                    (begin
                      (write '(funapp 327 21))
                      (display "\n")
                      ((lambda (j7608 k7609 f7610)
                         (letrec*
                          ((g7723
                            (lambda (g7607)
                              (letrec*
                               ((g7724
                                 (letrec*
                                  ((x7725
                                    (letrec*
                                     ((x7726
                                       (begin
                                         (write '(funapp 336 44))
                                         (display "\n")
                                         (pair?/c j7608 k7609 g7607))))
                                     (begin
                                       (write '(funapp 337 36))
                                       (display "\n")
                                       (f7610 x7726)))))
                                  (begin
                                    (write '(funapp 338 33))
                                    (display "\n")
                                    (any/c j7608 k7609 x7725)))))
                               g7724))))
                          g7723))
                       xj7605
                       xk7606
                       (lambda (p)
                         (letrec*
                          ((g7722
                            (begin
                              (write '(funapp 343 51))
                              (display "\n")
                              (orig-cdr p))))
                          g7722))))))
                  g7721)))
               (cons
                (letrec*
                 ((xj7611
                   (begin (write '(funapp 347 26)) (display "\n") 'server))
                  (xk7612
                   (begin (write '(funapp 347 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7727
                    (begin
                      (write '(funapp 350 21))
                      (display "\n")
                      ((lambda (j7615 k7616 f7617)
                         (letrec*
                          ((g7729
                            (lambda (g7613 g7614)
                              (letrec*
                               ((g7730
                                 (letrec*
                                  ((x7731
                                    (letrec*
                                     ((x7733
                                       (begin
                                         (write '(funapp 359 44))
                                         (display "\n")
                                         (any/c j7615 k7616 g7613)))
                                      (x7732
                                       (begin
                                         (write '(funapp 360 44))
                                         (display "\n")
                                         (any/c j7615 k7616 g7614))))
                                     (begin
                                       (write '(funapp 361 36))
                                       (display "\n")
                                       (f7617 x7733 x7732)))))
                                  (begin
                                    (write '(funapp 362 33))
                                    (display "\n")
                                    (pair?/c j7615 k7616 x7731)))))
                               g7730))))
                          g7729))
                       xj7611
                       xk7612
                       (lambda (a b)
                         (letrec*
                          ((g7728
                            (begin
                              (write '(funapp 368 41))
                              (display "\n")
                              (orig-cons a b))))
                          g7728))))))
                  g7727)))
               (vector-ref
                (letrec*
                 ((xj7618
                   (begin (write '(funapp 372 26)) (display "\n") 'server))
                  (xk7619
                   (begin (write '(funapp 372 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7734
                    (begin
                      (write '(funapp 375 21))
                      (display "\n")
                      ((lambda (j7621 k7622 f7623)
                         (letrec*
                          ((g7736
                            (lambda (g7620)
                              (letrec*
                               ((g7737
                                 (letrec*
                                  ((x7738
                                    (letrec*
                                     ((x7739
                                       (begin
                                         (write '(funapp 384 44))
                                         (display "\n")
                                         (vector?/c j7621 k7622 g7620))))
                                     (begin
                                       (write '(funapp 385 36))
                                       (display "\n")
                                       (f7623 x7739)))))
                                  (begin
                                    (write '(funapp 386 33))
                                    (display "\n")
                                    (integer?/c j7621 k7622 x7738)))))
                               g7737))))
                          g7736))
                       xj7618
                       xk7619
                       (lambda (v i)
                         (letrec*
                          ((g7735
                            (begin
                              (write '(funapp 392 41))
                              (display "\n")
                              (orig-vector-ref v i))))
                          g7735))))))
                  g7734)))
               (vector-set!
                (letrec*
                 ((xj7624
                   (begin (write '(funapp 396 26)) (display "\n") 'server))
                  (xk7625
                   (begin (write '(funapp 396 43)) (display "\n") 'client)))
                 (letrec*
                  ((g7740
                    (begin
                      (write '(funapp 399 21))
                      (display "\n")
                      ((lambda (j7628 k7629 f7630)
                         (letrec*
                          ((g7742
                            (lambda (g7626 g7627)
                              (letrec*
                               ((g7743
                                 (letrec*
                                  ((x7744
                                    (letrec*
                                     ((x7746
                                       (begin
                                         (write '(funapp 408 44))
                                         (display "\n")
                                         (vector?/c j7628 k7629 g7626)))
                                      (x7745
                                       (begin
                                         (write '(funapp 409 44))
                                         (display "\n")
                                         (integer?/c j7628 k7629 g7627))))
                                     (begin
                                       (write '(funapp 410 36))
                                       (display "\n")
                                       (f7630 x7746 x7745)))))
                                  (begin
                                    (write '(funapp 411 33))
                                    (display "\n")
                                    (any/c j7628 k7629 x7744)))))
                               g7743))))
                          g7742))
                       xj7624
                       xk7625
                       (lambda (vec i v)
                         (letrec*
                          ((g7741
                            (begin
                              (write '(funapp 417 41))
                              (display "\n")
                              (orig-vector-set! vec i v))))
                          g7741))))))
                  g7740)))
               (assert
                (lambda (cnd)
                  (letrec*
                   ((g7747
                     (if cnd
                       (begin (write '(funapp 422 35)) (display "\n") '())
                       (begin
                         (write '(funapp 422 40))
                         (display "\n")
                         (error "assertion failed")))))
                   g7747)))
               (cdddr
                (lambda (x)
                  (letrec*
                   ((g7748
                     (letrec*
                      ((x7749
                        (letrec*
                         ((x7750
                           (begin
                             (write '(funapp 429 48))
                             (display "\n")
                             (cdr x))))
                         (begin
                           (write '(funapp 429 58))
                           (display "\n")
                           (cdr x7750)))))
                      (begin
                        (write '(funapp 430 23))
                        (display "\n")
                        (cdr x7749)))))
                   g7748)))
               (__toplevel_set-car! set-car!)
               (map
                (lambda (f l)
                  (letrec*
                   ((g7751
                     (letrec*
                      ((x7754
                        (begin
                          (write '(funapp 436 45))
                          (display "\n")
                          (procedure? f))))
                      (begin
                        (write '(funapp 436 62))
                        (display "\n")
                        (assert x7754))))
                    (g7752
                     (letrec*
                      ((x7755
                        (begin
                          (write '(funapp 437 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 437 57))
                        (display "\n")
                        (assert x7755))))
                    (g7753
                     (letrec*
                      ((x-cnd7756
                        (begin
                          (write '(funapp 440 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd7756
                        (begin (write '(funapp 442 24)) (display "\n") '())
                        (letrec*
                         ((x7759
                           (letrec*
                            ((x7760
                              (begin
                                (write '(funapp 444 51))
                                (display "\n")
                                (car l))))
                            (begin
                              (write '(funapp 444 61))
                              (display "\n")
                              (f x7760))))
                          (x7757
                           (letrec*
                            ((x7758
                              (begin
                                (write '(funapp 445 51))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 445 61))
                              (display "\n")
                              (map f x7758)))))
                         (begin
                           (write '(funapp 446 26))
                           (display "\n")
                           (cons x7759 x7757)))))))
                   g7753)))
               (cdar
                (lambda (x)
                  (letrec*
                   ((g7761
                     (letrec*
                      ((x7762
                        (begin
                          (write '(funapp 451 45))
                          (display "\n")
                          (car x))))
                      (begin
                        (write '(funapp 451 55))
                        (display "\n")
                        (cdr x7762)))))
                   g7761)))
               (cadadr
                (lambda (x)
                  (letrec*
                   ((g7763
                     (letrec*
                      ((x7764
                        (letrec*
                         ((x7765
                           (letrec*
                            ((x7766
                              (begin
                                (write '(funapp 460 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 460 61))
                              (display "\n")
                              (car x7766)))))
                         (begin
                           (write '(funapp 461 26))
                           (display "\n")
                           (cdr x7765)))))
                      (begin
                        (write '(funapp 462 23))
                        (display "\n")
                        (car x7764)))))
                   g7763)))
               (cdadar
                (lambda (x)
                  (letrec*
                   ((g7767
                     (letrec*
                      ((x7768
                        (letrec*
                         ((x7769
                           (letrec*
                            ((x7770
                              (begin
                                (write '(funapp 471 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 471 61))
                              (display "\n")
                              (cdr x7770)))))
                         (begin
                           (write '(funapp 472 26))
                           (display "\n")
                           (car x7769)))))
                      (begin
                        (write '(funapp 473 23))
                        (display "\n")
                        (cdr x7768)))))
                   g7767)))
               (call-with-output-file
                (lambda (filename proc)
                  (letrec*
                   ((g7771
                     (letrec*
                      ((x7774
                        (begin
                          (write '(funapp 479 39))
                          (display "\n")
                          (string? filename))))
                      (begin
                        (write '(funapp 479 60))
                        (display "\n")
                        (assert x7774))))
                    (g7772
                     (letrec*
                      ((x7775
                        (begin
                          (write '(funapp 481 39))
                          (display "\n")
                          (procedure? proc))))
                      (begin
                        (write '(funapp 481 59))
                        (display "\n")
                        (assert x7775))))
                    (g7773
                     (letrec*
                      ((output-port
                        (begin
                          (write '(funapp 484 37))
                          (display "\n")
                          (open-output-file filename)))
                       (res
                        (begin
                          (write '(funapp 485 29))
                          (display "\n")
                          (proc output-port))))
                      (letrec*
                       ((g7776
                         (begin
                           (write '(funapp 487 32))
                           (display "\n")
                           (close-output-port output-port)))
                        (g7777 res))
                       g7777))))
                   g7773)))
               (caddr
                (lambda (x)
                  (letrec*
                   ((g7778
                     (letrec*
                      ((x7779
                        (letrec*
                         ((x7780
                           (begin
                             (write '(funapp 495 48))
                             (display "\n")
                             (cdr x))))
                         (begin
                           (write '(funapp 495 58))
                           (display "\n")
                           (cdr x7780)))))
                      (begin
                        (write '(funapp 496 23))
                        (display "\n")
                        (car x7779)))))
                   g7778)))
               (cdaadr
                (lambda (x)
                  (letrec*
                   ((g7781
                     (letrec*
                      ((x7782
                        (letrec*
                         ((x7783
                           (letrec*
                            ((x7784
                              (begin
                                (write '(funapp 505 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 505 61))
                              (display "\n")
                              (car x7784)))))
                         (begin
                           (write '(funapp 506 26))
                           (display "\n")
                           (car x7783)))))
                      (begin
                        (write '(funapp 507 23))
                        (display "\n")
                        (cdr x7782)))))
                   g7781)))
               (assq
                (lambda (k l)
                  (letrec*
                   ((g7785
                     (letrec*
                      ((x7787
                        (begin
                          (write '(funapp 512 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 512 57))
                        (display "\n")
                        (assert x7787))))
                    (g7786
                     (letrec*
                      ((x-cnd7788
                        (begin
                          (write '(funapp 515 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd7788
                        #f
                        (letrec*
                         ((x-cnd7789
                           (letrec*
                            ((x7790
                              (begin
                                (write '(funapp 520 45))
                                (display "\n")
                                (caar l))))
                            (begin
                              (write '(funapp 520 56))
                              (display "\n")
                              (eq? x7790 k)))))
                         (if x-cnd7789
                           (begin
                             (write '(funapp 522 28))
                             (display "\n")
                             (car l))
                           (letrec*
                            ((x7791
                              (begin
                                (write '(funapp 523 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 523 55))
                              (display "\n")
                              (assq k x7791)))))))))
                   g7786)))
               (even?
                (lambda (x)
                  (letrec*
                   ((g7792
                     (letrec*
                      ((x7793
                        (begin
                          (write '(funapp 528 45))
                          (display "\n")
                          (modulo x 2))))
                      (begin
                        (write '(funapp 528 60))
                        (display "\n")
                        (= 0 x7793)))))
                   g7792)))
               (list->string
                (lambda (l)
                  (letrec*
                   ((g7794
                     (letrec*
                      ((x7796
                        (begin
                          (write '(funapp 533 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 533 57))
                        (display "\n")
                        (assert x7796))))
                    (g7795
                     (letrec*
                      ((x-cnd7797
                        (begin
                          (write '(funapp 536 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd7797
                        ""
                        (letrec*
                         ((x7800
                           (letrec*
                            ((x7801
                              (begin
                                (write '(funapp 541 45))
                                (display "\n")
                                (car l))))
                            (begin
                              (write '(funapp 541 55))
                              (display "\n")
                              (char->string x7801))))
                          (x7798
                           (letrec*
                            ((x7799
                              (begin
                                (write '(funapp 543 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 543 55))
                              (display "\n")
                              (list->string x7799)))))
                         (begin
                           (write '(funapp 544 26))
                           (display "\n")
                           (string-append x7800 x7798)))))))
                   g7795)))
               (char<=?
                (lambda (c1 c2)
                  (letrec*
                   ((g7802
                     (letrec*
                      ((x7805
                        (begin
                          (write '(funapp 549 45))
                          (display "\n")
                          (char? c1))))
                      (begin
                        (write '(funapp 549 58))
                        (display "\n")
                        (assert x7805))))
                    (g7803
                     (letrec*
                      ((x7806
                        (begin
                          (write '(funapp 550 45))
                          (display "\n")
                          (char? c2))))
                      (begin
                        (write '(funapp 550 58))
                        (display "\n")
                        (assert x7806))))
                    (g7804
                     (letrec*
                      ((val7515
                        (begin
                          (write '(funapp 553 33))
                          (display "\n")
                          (char<? c1 c2))))
                      (letrec*
                       ((g7807
                         (if val7515
                           val7515
                           (begin
                             (write '(funapp 555 52))
                             (display "\n")
                             (char=? c1 c2)))))
                       g7807))))
                   g7804)))
               (cdddar
                (lambda (x)
                  (letrec*
                   ((g7808
                     (letrec*
                      ((x7809
                        (letrec*
                         ((x7810
                           (letrec*
                            ((x7811
                              (begin
                                (write '(funapp 565 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 565 61))
                              (display "\n")
                              (cdr x7811)))))
                         (begin
                           (write '(funapp 566 26))
                           (display "\n")
                           (cdr x7810)))))
                      (begin
                        (write '(funapp 567 23))
                        (display "\n")
                        (cdr x7809)))))
                   g7808)))
               (list-tail
                (lambda (x k)
                  (letrec*
                   ((g7812
                     (letrec*
                      ((x7815
                        (begin
                          (write '(funapp 572 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 572 57))
                        (display "\n")
                        (assert x7815))))
                    (g7813
                     (letrec*
                      ((x7816
                        (begin
                          (write '(funapp 573 45))
                          (display "\n")
                          (number?))))
                      (begin
                        (write '(funapp 573 57))
                        (display "\n")
                        (assert x7816))))
                    (g7814
                     (letrec*
                      ((x-cnd7817
                        (begin
                          (write '(funapp 576 35))
                          (display "\n")
                          (zero? k))))
                      (if x-cnd7817
                        x
                        (letrec*
                         ((x7819
                           (begin
                             (write '(funapp 580 34))
                             (display "\n")
                             (cdr x)))
                          (x7818
                           (begin
                             (write '(funapp 580 50))
                             (display "\n")
                             (- k 1))))
                         (begin
                           (write '(funapp 581 26))
                           (display "\n")
                           (list-tail x7819 x7818)))))))
                   g7814)))
               (halt
                (lambda ()
                  (letrec*
                   ((g7820
                     (begin (write '(funapp 583 49)) (display "\n") '())))
                   g7820)))
               (char-alphabetic?
                (lambda (c)
                  (letrec*
                   ((g7821
                     (letrec*
                      ((x-cnd7822
                        (letrec*
                         ((x7823 #\a))
                         (begin
                           (write '(funapp 590 48))
                           (display "\n")
                           (char-ci>=? c x7823)))))
                      (if x-cnd7822
                        (letrec*
                         ((x7824 #\z))
                         (begin
                           (write '(funapp 592 48))
                           (display "\n")
                           (char-ci<=? c x7824)))
                        #f))))
                   g7821)))
               (<=
                (lambda (x y)
                  (letrec*
                   ((g7825
                     (letrec*
                      ((x7827
                        (begin
                          (write '(funapp 598 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 598 59))
                        (display "\n")
                        (assert x7827))))
                    (g7826
                     (letrec*
                      ((val7516
                        (begin
                          (write '(funapp 601 33))
                          (display "\n")
                          (< x y))))
                      (letrec*
                       ((g7828
                         (if val7516
                           val7516
                           (letrec*
                            ((val7517
                              (begin
                                (write '(funapp 607 39))
                                (display "\n")
                                (= x y))))
                            (letrec*
                             ((g7829 (if val7517 val7517 #f)))
                             g7829)))))
                       g7828))))
                   g7826)))
               (char-whitespace?
                (lambda (c)
                  (letrec*
                   ((g7830
                     (letrec*
                      ((val7518
                        (letrec*
                         ((x7831
                           (begin
                             (write '(funapp 619 42))
                             (display "\n")
                             (char->integer c))))
                         (begin
                           (write '(funapp 619 62))
                           (display "\n")
                           (= x7831 9)))))
                      (letrec*
                       ((g7832
                         (if val7518
                           val7518
                           (letrec*
                            ((val7519
                              (letrec*
                               ((x7833
                                 (begin
                                   (write '(funapp 627 40))
                                   (display "\n")
                                   (char->integer c))))
                               (begin
                                 (write '(funapp 628 32))
                                 (display "\n")
                                 (= x7833 10)))))
                            (letrec*
                             ((g7834
                               (if val7519
                                 val7519
                                 (letrec*
                                  ((x7835
                                    (begin
                                      (write '(funapp 634 43))
                                      (display "\n")
                                      (char->integer c))))
                                  (begin
                                    (write '(funapp 635 35))
                                    (display "\n")
                                    (= x7835 32))))))
                             g7834)))))
                       g7832))))
                   g7830)))
               (cddar
                (lambda (x)
                  (letrec*
                   ((g7836
                     (letrec*
                      ((x7837
                        (letrec*
                         ((x7838
                           (begin
                             (write '(funapp 644 48))
                             (display "\n")
                             (car x))))
                         (begin
                           (write '(funapp 644 58))
                           (display "\n")
                           (cdr x7838)))))
                      (begin
                        (write '(funapp 645 23))
                        (display "\n")
                        (cdr x7837)))))
                   g7836)))
               (positive?
                (lambda (x)
                  (letrec*
                   ((g7839
                     (letrec*
                      ((x7841
                        (begin
                          (write '(funapp 650 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 650 59))
                        (display "\n")
                        (assert x7841))))
                    (g7840
                     (begin (write '(funapp 651 28)) (display "\n") (> x 0))))
                   g7840)))
               ($pc (begin (write '(funapp 653 20)) (display "\n") '()))
               (ref-set set-car!)
               (void (lambda () (letrec* ((g7842 #f)) g7842)))
               (cddr
                (lambda (x)
                  (letrec*
                   ((g7843
                     (letrec*
                      ((x7844
                        (begin
                          (write '(funapp 659 45))
                          (display "\n")
                          (cdr x))))
                      (begin
                        (write '(funapp 659 55))
                        (display "\n")
                        (cdr x7844)))))
                   g7843)))
               (truncate
                (lambda (x)
                  (letrec*
                   ((g7845
                     (letrec*
                      ((x7847
                        (begin
                          (write '(funapp 664 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 664 59))
                        (display "\n")
                        (assert x7847))))
                    (g7846
                     (letrec*
                      ((x-cnd7848
                        (begin
                          (write '(funapp 667 35))
                          (display "\n")
                          (< x 0))))
                      (if x-cnd7848
                        (begin
                          (write '(funapp 668 37))
                          (display "\n")
                          (ceiling x))
                        (begin
                          (write '(funapp 668 49))
                          (display "\n")
                          (floor x))))))
                   g7846)))
               ($cmp (begin (write '(funapp 670 21)) (display "\n") '()))
               (equal?
                (lambda (a b)
                  (letrec*
                   ((g7849
                     (letrec*
                      ((val7520
                        (begin
                          (write '(funapp 676 33))
                          (display "\n")
                          (eq? a b))))
                      (letrec*
                       ((g7850
                         (if val7520
                           val7520
                           (letrec*
                            ((val7521
                              (letrec*
                               ((x-cnd7851
                                 (begin
                                   (write '(funapp 684 44))
                                   (display "\n")
                                   (null? a))))
                               (if x-cnd7851
                                 (begin
                                   (write '(funapp 685 46))
                                   (display "\n")
                                   (null? b))
                                 #f))))
                            (letrec*
                             ((g7852
                               (if val7521
                                 val7521
                                 (letrec*
                                  ((val7522
                                    (letrec*
                                     ((x-cnd7853
                                       (begin
                                         (write '(funapp 693 50))
                                         (display "\n")
                                         (string? a))))
                                     (if x-cnd7853
                                       (letrec*
                                        ((x-cnd7854
                                          (begin
                                            (write '(funapp 696 53))
                                            (display "\n")
                                            (string? b))))
                                        (if x-cnd7854
                                          (begin
                                            (write '(funapp 697 55))
                                            (display "\n")
                                            (string=? a b))
                                          #f))
                                       #f))))
                                  (letrec*
                                   ((g7855
                                     (if val7522
                                       val7522
                                       (letrec*
                                        ((val7523
                                          (letrec*
                                           ((x-cnd7856
                                             (begin
                                               (write '(funapp 706 56))
                                               (display "\n")
                                               (pair? a))))
                                           (if x-cnd7856
                                             (letrec*
                                              ((x-cnd7857
                                                (begin
                                                  (write '(funapp 709 59))
                                                  (display "\n")
                                                  (pair? b))))
                                              (if x-cnd7857
                                                (letrec*
                                                 ((x-cnd7858
                                                   (letrec*
                                                    ((x7860
                                                      (begin
                                                        (write
                                                         '(funapp 714 61))
                                                        (display "\n")
                                                        (car a)))
                                                     (x7859
                                                      (begin
                                                        (write
                                                         '(funapp 715 61))
                                                        (display "\n")
                                                        (car b))))
                                                    (begin
                                                      (write '(funapp 716 53))
                                                      (display "\n")
                                                      (equal? x7860 x7859)))))
                                                 (if x-cnd7858
                                                   (letrec*
                                                    ((x7862
                                                      (begin
                                                        (write
                                                         '(funapp 719 61))
                                                        (display "\n")
                                                        (cdr a)))
                                                     (x7861
                                                      (begin
                                                        (write
                                                         '(funapp 720 61))
                                                        (display "\n")
                                                        (cdr b))))
                                                    (begin
                                                      (write '(funapp 721 53))
                                                      (display "\n")
                                                      (equal? x7862 x7861)))
                                                   #f))
                                                #f))
                                             #f))))
                                        (letrec*
                                         ((g7863
                                           (if val7523
                                             val7523
                                             (letrec*
                                              ((x-cnd7864
                                                (begin
                                                  (write '(funapp 730 59))
                                                  (display "\n")
                                                  (vector? a))))
                                              (if x-cnd7864
                                                (letrec*
                                                 ((x-cnd7865
                                                   (begin
                                                     (write '(funapp 733 62))
                                                     (display "\n")
                                                     (vector? b))))
                                                 (if x-cnd7865
                                                   (letrec*
                                                    ((n
                                                      (begin
                                                        (write
                                                         '(funapp 736 57))
                                                        (display "\n")
                                                        (vector-length a))))
                                                    (letrec*
                                                     ((g7866
                                                       (letrec*
                                                        ((x-cnd7867
                                                          (letrec*
                                                           ((x7868
                                                             (begin
                                                               (write
                                                                '(funapp
                                                                  743
                                                                  62))
                                                               (display "\n")
                                                               (vector-length
                                                                b))))
                                                           (begin
                                                             (write
                                                              '(funapp 745 60))
                                                             (display "\n")
                                                             (= x7868 n)))))
                                                        (if x-cnd7867
                                                          (letrec*
                                                           ((loop
                                                             (lambda (i)
                                                               (letrec*
                                                                ((g7869
                                                                  (letrec*
                                                                   ((val7524
                                                                     (begin
                                                                       (write
                                                                        '(funapp
                                                                          754
                                                                          70))
                                                                       (display
                                                                        "\n")
                                                                       (=
                                                                        i
                                                                        n))))
                                                                   (letrec*
                                                                    ((g7870
                                                                      (if val7524
                                                                        val7524
                                                                        (letrec*
                                                                         ((x-cnd7871
                                                                           (letrec*
                                                                            ((x7873
                                                                              (begin
                                                                                (write
                                                                                 '(funapp
                                                                                   763
                                                                                   79))
                                                                                (display
                                                                                 "\n")
                                                                                (vector-ref
                                                                                 a
                                                                                 i)))
                                                                             (x7872
                                                                              (begin
                                                                                (write
                                                                                 '(funapp
                                                                                   767
                                                                                   79))
                                                                                (display
                                                                                 "\n")
                                                                                (vector-ref
                                                                                 b
                                                                                 i))))
                                                                            (begin
                                                                              (write
                                                                               '(funapp
                                                                                 770
                                                                                 77))
                                                                              (display
                                                                               "\n")
                                                                              (equal?
                                                                               x7873
                                                                               x7872)))))
                                                                         (if x-cnd7871
                                                                           (letrec*
                                                                            ((x7874
                                                                              (begin
                                                                                (write
                                                                                 '(funapp
                                                                                   776
                                                                                   79))
                                                                                (display
                                                                                 "\n")
                                                                                (+
                                                                                 i
                                                                                 1))))
                                                                            (begin
                                                                              (write
                                                                               '(funapp
                                                                                 779
                                                                                 77))
                                                                              (display
                                                                               "\n")
                                                                              (loop
                                                                               x7874)))
                                                                           #f)))))
                                                                    g7870))))
                                                                g7869))))
                                                           (letrec*
                                                            ((g7875
                                                              (begin
                                                                (write
                                                                 '(funapp
                                                                   785
                                                                   69))
                                                                (display "\n")
                                                                (loop 0))))
                                                            g7875))
                                                          #f))))
                                                     g7866))
                                                   #f))
                                                #f)))))
                                         g7863)))))
                                   g7855)))))
                             g7852)))))
                       g7850))))
                   g7849)))
               (cdaaar
                (lambda (x)
                  (letrec*
                   ((g7876
                     (letrec*
                      ((x7877
                        (letrec*
                         ((x7878
                           (letrec*
                            ((x7879
                              (begin
                                (write '(funapp 803 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 803 61))
                              (display "\n")
                              (car x7879)))))
                         (begin
                           (write '(funapp 804 26))
                           (display "\n")
                           (car x7878)))))
                      (begin
                        (write '(funapp 805 23))
                        (display "\n")
                        (cdr x7877)))))
                   g7876)))
               (caaddr
                (lambda (x)
                  (letrec*
                   ((g7880
                     (letrec*
                      ((x7881
                        (letrec*
                         ((x7882
                           (letrec*
                            ((x7883
                              (begin
                                (write '(funapp 814 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 814 61))
                              (display "\n")
                              (cdr x7883)))))
                         (begin
                           (write '(funapp 815 26))
                           (display "\n")
                           (car x7882)))))
                      (begin
                        (write '(funapp 816 23))
                        (display "\n")
                        (car x7881)))))
                   g7880)))
               (eqv?
                (lambda (x y)
                  (letrec*
                   ((g7884
                     (begin
                       (write '(funapp 818 53))
                       (display "\n")
                       (eq? x y))))
                   g7884)))
               (>=
                (lambda (x y)
                  (letrec*
                   ((g7885
                     (letrec*
                      ((x7887
                        (begin
                          (write '(funapp 822 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 822 59))
                        (display "\n")
                        (assert x7887))))
                    (g7886
                     (letrec*
                      ((val7525
                        (begin
                          (write '(funapp 825 33))
                          (display "\n")
                          (> x y))))
                      (letrec*
                       ((g7888
                         (if val7525
                           val7525
                           (letrec*
                            ((val7526
                              (begin
                                (write '(funapp 831 39))
                                (display "\n")
                                (= x y))))
                            (letrec*
                             ((g7889 (if val7526 val7526 #f)))
                             g7889)))))
                       g7888))))
                   g7886)))
               (call-with-input-file
                (lambda (filename proc)
                  (letrec*
                   ((g7890
                     (letrec*
                      ((x7893
                        (begin
                          (write '(funapp 841 39))
                          (display "\n")
                          (string? filename))))
                      (begin
                        (write '(funapp 841 60))
                        (display "\n")
                        (assert x7893))))
                    (g7891
                     (letrec*
                      ((x7894
                        (begin
                          (write '(funapp 843 39))
                          (display "\n")
                          (procedure? proc))))
                      (begin
                        (write '(funapp 843 59))
                        (display "\n")
                        (assert x7894))))
                    (g7892
                     (letrec*
                      ((input-port
                        (begin
                          (write '(funapp 846 36))
                          (display "\n")
                          (open-input-file filename)))
                       (res
                        (begin
                          (write '(funapp 847 29))
                          (display "\n")
                          (proc input-port))))
                      (letrec*
                       ((g7895
                         (begin
                           (write '(funapp 849 32))
                           (display "\n")
                           (close-input-port input-port)))
                        (g7896 res))
                       g7896))))
                   g7892)))
               (ref
                (lambda (x)
                  (letrec*
                   ((g7897
                     (begin
                       (write '(funapp 852 50))
                       (display "\n")
                       (cons
                        x
                        (begin (write '(funapp 852 57)) (display "\n") '())))))
                   g7897)))
               (char>=?
                (lambda (c1 c2)
                  (letrec*
                   ((g7898
                     (letrec*
                      ((x7901
                        (begin
                          (write '(funapp 856 45))
                          (display "\n")
                          (char? c1))))
                      (begin
                        (write '(funapp 856 58))
                        (display "\n")
                        (assert x7901))))
                    (g7899
                     (letrec*
                      ((x7902
                        (begin
                          (write '(funapp 857 45))
                          (display "\n")
                          (char? c2))))
                      (begin
                        (write '(funapp 857 58))
                        (display "\n")
                        (assert x7902))))
                    (g7900
                     (letrec*
                      ((val7527
                        (begin
                          (write '(funapp 860 33))
                          (display "\n")
                          (char>? c1 c2))))
                      (letrec*
                       ((g7903
                         (if val7527
                           val7527
                           (begin
                             (write '(funapp 862 52))
                             (display "\n")
                             (char=? c1 c2)))))
                       g7903))))
                   g7900)))
               (cdaar
                (lambda (x)
                  (letrec*
                   ((g7904
                     (letrec*
                      ((x7905
                        (letrec*
                         ((x7906
                           (begin
                             (write '(funapp 870 48))
                             (display "\n")
                             (car x))))
                         (begin
                           (write '(funapp 870 58))
                           (display "\n")
                           (car x7906)))))
                      (begin
                        (write '(funapp 871 23))
                        (display "\n")
                        (cdr x7905)))))
                   g7904)))
               (cdaddr
                (lambda (x)
                  (letrec*
                   ((g7907
                     (letrec*
                      ((x7908
                        (letrec*
                         ((x7909
                           (letrec*
                            ((x7910
                              (begin
                                (write '(funapp 880 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 880 61))
                              (display "\n")
                              (cdr x7910)))))
                         (begin
                           (write '(funapp 881 26))
                           (display "\n")
                           (car x7909)))))
                      (begin
                        (write '(funapp 882 23))
                        (display "\n")
                        (cdr x7908)))))
                   g7907)))
               (__toplevel_cdr cdr)
               (cadar
                (lambda (x)
                  (letrec*
                   ((g7911
                     (letrec*
                      ((x7912
                        (letrec*
                         ((x7913
                           (begin
                             (write '(funapp 890 48))
                             (display "\n")
                             (car x))))
                         (begin
                           (write '(funapp 890 58))
                           (display "\n")
                           (cdr x7913)))))
                      (begin
                        (write '(funapp 891 23))
                        (display "\n")
                        (car x7912)))))
                   g7911)))
               (caadr
                (lambda (x)
                  (letrec*
                   ((g7914
                     (letrec*
                      ((x7915
                        (letrec*
                         ((x7916
                           (begin
                             (write '(funapp 898 48))
                             (display "\n")
                             (cdr x))))
                         (begin
                           (write '(funapp 898 58))
                           (display "\n")
                           (car x7916)))))
                      (begin
                        (write '(funapp 899 23))
                        (display "\n")
                        (car x7915)))))
                   g7914)))
               (char-ci>?
                (lambda (c1 c2)
                  (letrec*
                   ((g7917
                     (letrec*
                      ((x7920
                        (begin
                          (write '(funapp 904 45))
                          (display "\n")
                          (char? c1))))
                      (begin
                        (write '(funapp 904 58))
                        (display "\n")
                        (assert x7920))))
                    (g7918
                     (letrec*
                      ((x7921
                        (begin
                          (write '(funapp 905 45))
                          (display "\n")
                          (char? c2))))
                      (begin
                        (write '(funapp 905 58))
                        (display "\n")
                        (assert x7921))))
                    (g7919
                     (letrec*
                      ((x7922
                        (begin
                          (write '(funapp 906 45))
                          (display "\n")
                          (char-ci<=? c1 c2))))
                      (begin
                        (write '(funapp 906 66))
                        (display "\n")
                        (not x7922)))))
                   g7919)))
               (__toplevel_set-cdr! set-cdr!)
               (caaaar
                (lambda (x)
                  (letrec*
                   ((g7923
                     (letrec*
                      ((x7924
                        (letrec*
                         ((x7925
                           (letrec*
                            ((x7926
                              (begin
                                (write '(funapp 916 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 916 61))
                              (display "\n")
                              (car x7926)))))
                         (begin
                           (write '(funapp 917 26))
                           (display "\n")
                           (car x7925)))))
                      (begin
                        (write '(funapp 918 23))
                        (display "\n")
                        (car x7924)))))
                   g7923)))
               (negative?
                (lambda (x)
                  (letrec*
                   ((g7927
                     (letrec*
                      ((x7929
                        (begin
                          (write '(funapp 923 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 923 59))
                        (display "\n")
                        (assert x7929))))
                    (g7928
                     (begin (write '(funapp 924 28)) (display "\n") (< x 0))))
                   g7928)))
               (memv
                (lambda (e l)
                  (letrec*
                   ((g7930
                     (begin
                       (write '(funapp 926 53))
                       (display "\n")
                       (memq e l))))
                   g7930)))
               (caaar
                (lambda (x)
                  (letrec*
                   ((g7931
                     (letrec*
                      ((x7932
                        (letrec*
                         ((x7933
                           (begin
                             (write '(funapp 932 48))
                             (display "\n")
                             (car x))))
                         (begin
                           (write '(funapp 932 58))
                           (display "\n")
                           (car x7933)))))
                      (begin
                        (write '(funapp 933 23))
                        (display "\n")
                        (car x7932)))))
                   g7931)))
               (debug
                (lambda (e)
                  (letrec*
                   ((g7934
                     (begin (write '(funapp 935 51)) (display "\n") '())))
                   g7934)))
               (reverse
                (lambda (l)
                  (letrec*
                   ((g7935
                     (letrec*
                      ((x7937
                        (begin
                          (write '(funapp 939 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 939 57))
                        (display "\n")
                        (assert x7937))))
                    (g7936
                     (letrec*
                      ((x-cnd7938
                        (begin
                          (write '(funapp 942 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd7938
                        (begin (write '(funapp 944 24)) (display "\n") '())
                        (letrec*
                         ((x7941
                           (letrec*
                            ((x7942
                              (begin
                                (write '(funapp 946 51))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 946 61))
                              (display "\n")
                              (reverse x7942))))
                          (x7939
                           (letrec*
                            ((x7940
                              (begin
                                (write '(funapp 947 51))
                                (display "\n")
                                (car l))))
                            (begin
                              (write '(funapp 947 61))
                              (display "\n")
                              (list x7940)))))
                         (begin
                           (write '(funapp 948 26))
                           (display "\n")
                           (append x7941 x7939)))))))
                   g7936)))
               (caaadr
                (lambda (x)
                  (letrec*
                   ((g7943
                     (letrec*
                      ((x7944
                        (letrec*
                         ((x7945
                           (letrec*
                            ((x7946
                              (begin
                                (write '(funapp 957 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 957 61))
                              (display "\n")
                              (car x7946)))))
                         (begin
                           (write '(funapp 958 26))
                           (display "\n")
                           (car x7945)))))
                      (begin
                        (write '(funapp 959 23))
                        (display "\n")
                        (car x7944)))))
                   g7943)))
               (cddadr
                (lambda (x)
                  (letrec*
                   ((g7947
                     (letrec*
                      ((x7948
                        (letrec*
                         ((x7949
                           (letrec*
                            ((x7950
                              (begin
                                (write '(funapp 968 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 968 61))
                              (display "\n")
                              (car x7950)))))
                         (begin
                           (write '(funapp 969 26))
                           (display "\n")
                           (cdr x7949)))))
                      (begin
                        (write '(funapp 970 23))
                        (display "\n")
                        (cdr x7948)))))
                   g7947)))
               (odd?
                (lambda (x)
                  (letrec*
                   ((g7951
                     (letrec*
                      ((x7953
                        (begin
                          (write '(funapp 975 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 975 59))
                        (display "\n")
                        (assert x7953))))
                    (g7952
                     (letrec*
                      ((x7954
                        (begin
                          (write '(funapp 976 45))
                          (display "\n")
                          (modulo x 2))))
                      (begin
                        (write '(funapp 976 60))
                        (display "\n")
                        (= 1 x7954)))))
                   g7952)))
               (caadar
                (lambda (x)
                  (letrec*
                   ((g7955
                     (letrec*
                      ((x7956
                        (letrec*
                         ((x7957
                           (letrec*
                            ((x7958
                              (begin
                                (write '(funapp 985 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 985 61))
                              (display "\n")
                              (cdr x7958)))))
                         (begin
                           (write '(funapp 986 26))
                           (display "\n")
                           (car x7957)))))
                      (begin
                        (write '(funapp 987 23))
                        (display "\n")
                        (car x7956)))))
                   g7955)))
               (apply
                (lambda (proc args)
                  (letrec*
                   ((g7959
                     (letrec*
                      ((x7962
                        (begin
                          (write '(funapp 993 39))
                          (display "\n")
                          (procedure? proc))))
                      (begin
                        (write '(funapp 993 59))
                        (display "\n")
                        (assert x7962))))
                    (g7960
                     (letrec*
                      ((x7963
                        (begin
                          (write '(funapp 994 45))
                          (display "\n")
                          (list? args))))
                      (begin
                        (write '(funapp 994 60))
                        (display "\n")
                        (assert x7963))))
                    (g7961
                     (letrec*
                      ((x-cnd7964
                        (begin
                          (write '(funapp 997 35))
                          (display "\n")
                          (null? args))))
                      (if x-cnd7964
                        (letrec*
                         ((g7965
                           (begin
                             (write '(funapp 999 42))
                             (display "\n")
                             (proc))))
                         g7965)
                        (letrec*
                         ((x-cnd7966
                           (letrec*
                            ((x7967
                              (begin
                                (write '(funapp 1002 45))
                                (display "\n")
                                (cdr args))))
                            (begin
                              (write '(funapp 1002 58))
                              (display "\n")
                              (null? x7967)))))
                         (if x-cnd7966
                           (letrec*
                            ((g7968
                              (letrec*
                               ((x7969
                                 (begin
                                   (write '(funapp 1006 48))
                                   (display "\n")
                                   (car args))))
                               (begin
                                 (write '(funapp 1006 61))
                                 (display "\n")
                                 (proc x7969)))))
                            g7968)
                           (letrec*
                            ((x-cnd7970
                              (letrec*
                               ((x7971
                                 (begin
                                   (write '(funapp 1010 48))
                                   (display "\n")
                                   (cddr args))))
                               (begin
                                 (write '(funapp 1010 62))
                                 (display "\n")
                                 (null? x7971)))))
                            (if x-cnd7970
                              (letrec*
                               ((g7972
                                 (letrec*
                                  ((x7974
                                    (begin
                                      (write '(funapp 1015 43))
                                      (display "\n")
                                      (car args)))
                                   (x7973
                                    (begin
                                      (write '(funapp 1015 62))
                                      (display "\n")
                                      (cadr args))))
                                  (begin
                                    (write '(funapp 1016 35))
                                    (display "\n")
                                    (proc x7974 x7973)))))
                               g7972)
                              (letrec*
                               ((x-cnd7975
                                 (letrec*
                                  ((x7976
                                    (begin
                                      (write '(funapp 1021 43))
                                      (display "\n")
                                      (cdddr args))))
                                  (begin
                                    (write '(funapp 1022 35))
                                    (display "\n")
                                    (null? x7976)))))
                               (if x-cnd7975
                                 (letrec*
                                  ((g7977
                                    (letrec*
                                     ((x7980
                                       (begin
                                         (write '(funapp 1027 46))
                                         (display "\n")
                                         (car args)))
                                      (x7979
                                       (begin
                                         (write '(funapp 1028 46))
                                         (display "\n")
                                         (cadr args)))
                                      (x7978
                                       (begin
                                         (write '(funapp 1029 46))
                                         (display "\n")
                                         (caddr args))))
                                     (begin
                                       (write '(funapp 1030 38))
                                       (display "\n")
                                       (proc x7980 x7979 x7978)))))
                                  g7977)
                                 (letrec*
                                  ((x-cnd7981
                                    (letrec*
                                     ((x7982
                                       (begin
                                         (write '(funapp 1035 46))
                                         (display "\n")
                                         (cddddr args))))
                                     (begin
                                       (write '(funapp 1036 38))
                                       (display "\n")
                                       (null? x7982)))))
                                  (if x-cnd7981
                                    (letrec*
                                     ((g7983
                                       (letrec*
                                        ((x7987
                                          (begin
                                            (write '(funapp 1041 49))
                                            (display "\n")
                                            (car args)))
                                         (x7986
                                          (begin
                                            (write '(funapp 1042 49))
                                            (display "\n")
                                            (cadr args)))
                                         (x7985
                                          (begin
                                            (write '(funapp 1043 49))
                                            (display "\n")
                                            (caddr args)))
                                         (x7984
                                          (begin
                                            (write '(funapp 1044 49))
                                            (display "\n")
                                            (cadddr args))))
                                        (begin
                                          (write '(funapp 1045 41))
                                          (display "\n")
                                          (proc x7987 x7986 x7985 x7984)))))
                                     g7983)
                                    (letrec*
                                     ((x-cnd7988
                                       (letrec*
                                        ((x7989
                                          (letrec*
                                           ((x7990
                                             (begin
                                               (write '(funapp 1052 52))
                                               (display "\n")
                                               (cddddr args))))
                                           (begin
                                             (write '(funapp 1053 44))
                                             (display "\n")
                                             (cdr x7990)))))
                                        (begin
                                          (write '(funapp 1054 41))
                                          (display "\n")
                                          (null? x7989)))))
                                     (if x-cnd7988
                                       (letrec*
                                        ((g7991
                                          (letrec*
                                           ((x7997
                                             (begin
                                               (write '(funapp 1059 52))
                                               (display "\n")
                                               (car args)))
                                            (x7996
                                             (begin
                                               (write '(funapp 1060 52))
                                               (display "\n")
                                               (cadr args)))
                                            (x7995
                                             (begin
                                               (write '(funapp 1061 52))
                                               (display "\n")
                                               (caddr args)))
                                            (x7994
                                             (begin
                                               (write '(funapp 1062 52))
                                               (display "\n")
                                               (cadddr args)))
                                            (x7992
                                             (letrec*
                                              ((x7993
                                                (begin
                                                  (write '(funapp 1065 55))
                                                  (display "\n")
                                                  (cddddr args))))
                                              (begin
                                                (write '(funapp 1066 47))
                                                (display "\n")
                                                (car x7993)))))
                                           (begin
                                             (write '(funapp 1067 44))
                                             (display "\n")
                                             (proc
                                              x7997
                                              x7996
                                              x7995
                                              x7994
                                              x7992)))))
                                        g7991)
                                       (letrec*
                                        ((x-cnd7998
                                          (letrec*
                                           ((x7999
                                             (letrec*
                                              ((x8000
                                                (begin
                                                  (write '(funapp 1079 55))
                                                  (display "\n")
                                                  (cddddr args))))
                                              (begin
                                                (write '(funapp 1080 47))
                                                (display "\n")
                                                (cddr x8000)))))
                                           (begin
                                             (write '(funapp 1081 44))
                                             (display "\n")
                                             (null? x7999)))))
                                        (if x-cnd7998
                                          (letrec*
                                           ((g8001
                                             (letrec*
                                              ((x8009
                                                (begin
                                                  (write '(funapp 1086 55))
                                                  (display "\n")
                                                  (car args)))
                                               (x8008
                                                (begin
                                                  (write '(funapp 1087 55))
                                                  (display "\n")
                                                  (cadr args)))
                                               (x8007
                                                (begin
                                                  (write '(funapp 1088 55))
                                                  (display "\n")
                                                  (caddr args)))
                                               (x8006
                                                (begin
                                                  (write '(funapp 1089 55))
                                                  (display "\n")
                                                  (cadddr args)))
                                               (x8004
                                                (letrec*
                                                 ((x8005
                                                   (begin
                                                     (write '(funapp 1092 58))
                                                     (display "\n")
                                                     (cddddr args))))
                                                 (begin
                                                   (write '(funapp 1093 50))
                                                   (display "\n")
                                                   (car x8005))))
                                               (x8002
                                                (letrec*
                                                 ((x8003
                                                   (begin
                                                     (write '(funapp 1096 58))
                                                     (display "\n")
                                                     (cddddr args))))
                                                 (begin
                                                   (write '(funapp 1097 50))
                                                   (display "\n")
                                                   (cadr x8003)))))
                                              (begin
                                                (write '(funapp 1098 47))
                                                (display "\n")
                                                (proc
                                                 x8009
                                                 x8008
                                                 x8007
                                                 x8006
                                                 x8004
                                                 x8002)))))
                                           g8001)
                                          (letrec*
                                           ((x-cnd8010
                                             (letrec*
                                              ((x8011
                                                (letrec*
                                                 ((x8012
                                                   (begin
                                                     (write '(funapp 1111 58))
                                                     (display "\n")
                                                     (cddddr args))))
                                                 (begin
                                                   (write '(funapp 1112 50))
                                                   (display "\n")
                                                   (cdddr x8012)))))
                                              (begin
                                                (write '(funapp 1113 47))
                                                (display "\n")
                                                (null? x8011)))))
                                           (if x-cnd8010
                                             (letrec*
                                              ((g8013
                                                (letrec*
                                                 ((x8023
                                                   (begin
                                                     (write '(funapp 1118 58))
                                                     (display "\n")
                                                     (car args)))
                                                  (x8022
                                                   (begin
                                                     (write '(funapp 1119 58))
                                                     (display "\n")
                                                     (cadr args)))
                                                  (x8021
                                                   (begin
                                                     (write '(funapp 1120 58))
                                                     (display "\n")
                                                     (caddr args)))
                                                  (x8020
                                                   (begin
                                                     (write '(funapp 1121 58))
                                                     (display "\n")
                                                     (cadddr args)))
                                                  (x8018
                                                   (letrec*
                                                    ((x8019
                                                      (begin
                                                        (write
                                                         '(funapp 1124 61))
                                                        (display "\n")
                                                        (cddddr args))))
                                                    (begin
                                                      (write '(funapp 1125 53))
                                                      (display "\n")
                                                      (car x8019))))
                                                  (x8016
                                                   (letrec*
                                                    ((x8017
                                                      (begin
                                                        (write
                                                         '(funapp 1128 61))
                                                        (display "\n")
                                                        (cddddr args))))
                                                    (begin
                                                      (write '(funapp 1129 53))
                                                      (display "\n")
                                                      (cadr x8017))))
                                                  (x8014
                                                   (letrec*
                                                    ((x8015
                                                      (begin
                                                        (write
                                                         '(funapp 1132 61))
                                                        (display "\n")
                                                        (cddddr args))))
                                                    (begin
                                                      (write '(funapp 1133 53))
                                                      (display "\n")
                                                      (caddr x8015)))))
                                                 (begin
                                                   (write '(funapp 1134 50))
                                                   (display "\n")
                                                   (proc
                                                    x8023
                                                    x8022
                                                    x8021
                                                    x8020
                                                    x8018
                                                    x8016
                                                    x8014)))))
                                              g8013)
                                             (letrec*
                                              ((g8024
                                                (begin
                                                  (write '(funapp 1145 49))
                                                  (display "\n")
                                                  (error
                                                   "Unsupported call."))))
                                              g8024)))))))))))))))))))
                   g7961)))
               (member
                (lambda (e l)
                  (letrec*
                   ((g8025
                     (letrec*
                      ((x8027
                        (begin
                          (write '(funapp 1151 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1151 57))
                        (display "\n")
                        (assert x8027))))
                    (g8026
                     (letrec*
                      ((x-cnd8028
                        (begin
                          (write '(funapp 1154 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd8028
                        #f
                        (letrec*
                         ((x-cnd8029
                           (letrec*
                            ((x8030
                              (begin
                                (write '(funapp 1159 45))
                                (display "\n")
                                (car l))))
                            (begin
                              (write '(funapp 1159 55))
                              (display "\n")
                              (equal? x8030 e)))))
                         (if x-cnd8029
                           l
                           (letrec*
                            ((x8031
                              (begin
                                (write '(funapp 1162 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 1162 55))
                              (display "\n")
                              (member e x8031)))))))))
                   g8026)))
               (cddddr
                (lambda (x)
                  (letrec*
                   ((g8032
                     (letrec*
                      ((x8033
                        (letrec*
                         ((x8034
                           (letrec*
                            ((x8035
                              (begin
                                (write '(funapp 1171 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 1171 61))
                              (display "\n")
                              (cdr x8035)))))
                         (begin
                           (write '(funapp 1172 26))
                           (display "\n")
                           (cdr x8034)))))
                      (begin
                        (write '(funapp 1173 23))
                        (display "\n")
                        (cdr x8033)))))
                   g8032)))
               (cadddr
                (lambda (x)
                  (letrec*
                   ((g8036
                     (letrec*
                      ((x8037
                        (letrec*
                         ((x8038
                           (letrec*
                            ((x8039
                              (begin
                                (write '(funapp 1182 51))
                                (display "\n")
                                (cdr x))))
                            (begin
                              (write '(funapp 1182 61))
                              (display "\n")
                              (cdr x8039)))))
                         (begin
                           (write '(funapp 1183 26))
                           (display "\n")
                           (cdr x8038)))))
                      (begin
                        (write '(funapp 1184 23))
                        (display "\n")
                        (car x8037)))))
                   g8036)))
               (int-top
                (lambda ()
                  (letrec*
                   ((g8040
                     (begin
                       (write '(funapp 1186 53))
                       (display "\n")
                       (random 42))))
                   g8040)))
               (zero?
                (lambda (x)
                  (letrec*
                   ((g8041
                     (letrec*
                      ((x8043
                        (begin
                          (write '(funapp 1190 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 1190 59))
                        (display "\n")
                        (assert x8043))))
                    (g8042
                     (begin (write '(funapp 1191 28)) (display "\n") (= x 0))))
                   g8042)))
               (string>=?
                (lambda (s1 s2)
                  (letrec*
                   ((g8044
                     (letrec*
                      ((val7528
                        (begin
                          (write '(funapp 1198 33))
                          (display "\n")
                          (string>? s1 s2))))
                      (letrec*
                       ((g8045
                         (if val7528
                           val7528
                           (begin
                             (write '(funapp 1200 52))
                             (display "\n")
                             (string=? s1 s2)))))
                       g8045))))
                   g8044)))
               (cadr
                (lambda (x)
                  (letrec*
                   ((g8046
                     (letrec*
                      ((x8047
                        (begin
                          (write '(funapp 1206 45))
                          (display "\n")
                          (cdr x))))
                      (begin
                        (write '(funapp 1206 55))
                        (display "\n")
                        (car x8047)))))
                   g8046)))
               (__toplevel_car car)
               (list?
                (lambda (l)
                  (letrec*
                   ((g8048
                     (letrec*
                      ((val7529
                        (letrec*
                         ((x-cnd8049
                           (begin
                             (write '(funapp 1216 38))
                             (display "\n")
                             (pair? l))))
                         (if x-cnd8049
                           (letrec*
                            ((x8050
                              (begin
                                (write '(funapp 1218 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 1218 55))
                              (display "\n")
                              (list? x8050)))
                           #f))))
                      (letrec*
                       ((g8051
                         (if val7529
                           val7529
                           (begin
                             (write '(funapp 1221 52))
                             (display "\n")
                             (null? l)))))
                       g8051))))
                   g8048)))
               (cddaar
                (lambda (x)
                  (letrec*
                   ((g8052
                     (letrec*
                      ((x8053
                        (letrec*
                         ((x8054
                           (letrec*
                            ((x8055
                              (begin
                                (write '(funapp 1231 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 1231 61))
                              (display "\n")
                              (car x8055)))))
                         (begin
                           (write '(funapp 1232 26))
                           (display "\n")
                           (cdr x8054)))))
                      (begin
                        (write '(funapp 1233 23))
                        (display "\n")
                        (cdr x8053)))))
                   g8052)))
               (char-numeric?
                (lambda (c)
                  (letrec*
                   ((g8056
                     (letrec*
                      ((x-cnd8057
                        (letrec*
                         ((x8058 #\0))
                         (begin
                           (write '(funapp 1240 58))
                           (display "\n")
                           (char<=? x8058 c)))))
                      (if x-cnd8057
                        (letrec*
                         ((x8059 #\9))
                         (begin
                           (write '(funapp 1242 48))
                           (display "\n")
                           (char<=? c x8059)))
                        #f))))
                   g8056)))
               (__toplevel_cons cons)
               (assv
                (lambda (k l)
                  (letrec*
                   ((g8060
                     (letrec*
                      ((x8062
                        (begin
                          (write '(funapp 1249 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1249 57))
                        (display "\n")
                        (assert x8062))))
                    (g8061
                     (letrec*
                      ((x-cnd8063
                        (begin
                          (write '(funapp 1252 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd8063
                        #f
                        (letrec*
                         ((x-cnd8064
                           (letrec*
                            ((x8065
                              (begin
                                (write '(funapp 1257 45))
                                (display "\n")
                                (caar l))))
                            (begin
                              (write '(funapp 1257 56))
                              (display "\n")
                              (eqv? x8065 k)))))
                         (if x-cnd8064
                           (begin
                             (write '(funapp 1259 28))
                             (display "\n")
                             (car l))
                           (letrec*
                            ((x8066
                              (begin
                                (write '(funapp 1260 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 1260 55))
                              (display "\n")
                              (assq k x8066)))))))))
                   g8061)))
               (not (lambda (x) (letrec* ((g8067 (if x #f #t))) g8067)))
               (__toplevel_append
                (lambda (l1 l2)
                  (letrec*
                   ((g8068
                     (begin
                       (write '(funapp 1264 50))
                       (display "\n")
                       (append l1 l2))))
                   g8068)))
               (memq
                (lambda (e l)
                  (letrec*
                   ((g8069
                     (letrec*
                      ((x8071
                        (begin
                          (write '(funapp 1268 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1268 57))
                        (display "\n")
                        (assert x8071))))
                    (g8070
                     (letrec*
                      ((x-cnd8072
                        (begin
                          (write '(funapp 1271 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd8072
                        #f
                        (letrec*
                         ((x-cnd8073
                           (letrec*
                            ((x8074
                              (begin
                                (write '(funapp 1276 45))
                                (display "\n")
                                (car l))))
                            (begin
                              (write '(funapp 1276 55))
                              (display "\n")
                              (eq? x8074 e)))))
                         (if x-cnd8073
                           l
                           (letrec*
                            ((x8075
                              (begin
                                (write '(funapp 1279 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 1279 55))
                              (display "\n")
                              (memq e x8075)))))))))
                   g8070)))
               (cadaar
                (lambda (x)
                  (letrec*
                   ((g8076
                     (letrec*
                      ((x8077
                        (letrec*
                         ((x8078
                           (letrec*
                            ((x8079
                              (begin
                                (write '(funapp 1288 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 1288 61))
                              (display "\n")
                              (car x8079)))))
                         (begin
                           (write '(funapp 1289 26))
                           (display "\n")
                           (cdr x8078)))))
                      (begin
                        (write '(funapp 1290 23))
                        (display "\n")
                        (car x8077)))))
                   g8076)))
               (length
                (lambda (l)
                  (letrec*
                   ((g8080
                     (letrec*
                      ((x8082
                        (begin
                          (write '(funapp 1295 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1295 57))
                        (display "\n")
                        (assert x8082))))
                    (g8081
                     (letrec*
                      ((rec
                        (lambda (l)
                          (letrec*
                           ((g8083
                             (letrec*
                              ((x-cnd8084
                                (begin
                                  (write '(funapp 1303 43))
                                  (display "\n")
                                  (null? l))))
                              (if x-cnd8084
                                0
                                (letrec*
                                 ((x8085
                                   (letrec*
                                    ((x8086
                                      (begin
                                        (write '(funapp 1308 53))
                                        (display "\n")
                                        (cdr l))))
                                    (begin
                                      (write '(funapp 1308 63))
                                      (display "\n")
                                      (rec x8086)))))
                                 (begin
                                   (write '(funapp 1309 34))
                                   (display "\n")
                                   (+ 1 x8085)))))))
                           g8083))))
                      (letrec*
                       ((g8087
                         (begin
                           (write '(funapp 1311 40))
                           (display "\n")
                           (rec l))))
                       g8087))))
                   g8081)))
               (char-ci<=?
                (lambda (c1 c2)
                  (letrec*
                   ((g8088
                     (letrec*
                      ((x8091
                        (begin
                          (write '(funapp 1316 45))
                          (display "\n")
                          (char? c1))))
                      (begin
                        (write '(funapp 1316 58))
                        (display "\n")
                        (assert x8091))))
                    (g8089
                     (letrec*
                      ((x8092
                        (begin
                          (write '(funapp 1317 45))
                          (display "\n")
                          (char? c2))))
                      (begin
                        (write '(funapp 1317 58))
                        (display "\n")
                        (assert x8092))))
                    (g8090
                     (letrec*
                      ((val7530
                        (begin
                          (write '(funapp 1320 33))
                          (display "\n")
                          (char-ci<? c1 c2))))
                      (letrec*
                       ((g8093
                         (if val7530
                           val7530
                           (begin
                             (write '(funapp 1322 52))
                             (display "\n")
                             (char-ci=? c1 c2)))))
                       g8093))))
                   g8090)))
               (string>?
                (lambda (s1 s2)
                  (letrec*
                   ((g8094
                     (letrec*
                      ((x8095
                        (begin
                          (write '(funapp 1328 45))
                          (display "\n")
                          (string<=? s1 s2))))
                      (begin
                        (write '(funapp 1328 65))
                        (display "\n")
                        (not x8095)))))
                   g8094)))
               (cdadr
                (lambda (x)
                  (letrec*
                   ((g8096
                     (letrec*
                      ((x8097
                        (letrec*
                         ((x8098
                           (begin
                             (write '(funapp 1335 48))
                             (display "\n")
                             (cdr x))))
                         (begin
                           (write '(funapp 1335 58))
                           (display "\n")
                           (car x8098)))))
                      (begin
                        (write '(funapp 1336 23))
                        (display "\n")
                        (cdr x8097)))))
                   g8096)))
               (assoc
                (lambda (k l)
                  (letrec*
                   ((g8099
                     (letrec*
                      ((x8101
                        (begin
                          (write '(funapp 1341 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1341 57))
                        (display "\n")
                        (assert x8101))))
                    (g8100
                     (letrec*
                      ((x-cnd8102
                        (begin
                          (write '(funapp 1344 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd8102
                        #f
                        (letrec*
                         ((x-cnd8103
                           (letrec*
                            ((x8104
                              (begin
                                (write '(funapp 1349 45))
                                (display "\n")
                                (caar l))))
                            (begin
                              (write '(funapp 1349 56))
                              (display "\n")
                              (equal? x8104 k)))))
                         (if x-cnd8103
                           (begin
                             (write '(funapp 1351 28))
                             (display "\n")
                             (car l))
                           (letrec*
                            ((x8105
                              (begin
                                (write '(funapp 1352 45))
                                (display "\n")
                                (cdr l))))
                            (begin
                              (write '(funapp 1352 55))
                              (display "\n")
                              (assoc k x8105)))))))))
                   g8100)))
               (caar
                (lambda (x)
                  (letrec*
                   ((g8106
                     (letrec*
                      ((x8107
                        (begin
                          (write '(funapp 1357 45))
                          (display "\n")
                          (car x))))
                      (begin
                        (write '(funapp 1357 55))
                        (display "\n")
                        (car x8107)))))
                   g8106)))
               (char>?
                (lambda (c1 c2)
                  (letrec*
                   ((g8108
                     (letrec*
                      ((x8111
                        (begin
                          (write '(funapp 1362 45))
                          (display "\n")
                          (char? c1))))
                      (begin
                        (write '(funapp 1362 58))
                        (display "\n")
                        (assert x8111))))
                    (g8109
                     (letrec*
                      ((x8112
                        (begin
                          (write '(funapp 1363 45))
                          (display "\n")
                          (char? c2))))
                      (begin
                        (write '(funapp 1363 58))
                        (display "\n")
                        (assert x8112))))
                    (g8110
                     (letrec*
                      ((x8113
                        (begin
                          (write '(funapp 1364 45))
                          (display "\n")
                          (char<=? c1 c2))))
                      (begin
                        (write '(funapp 1364 63))
                        (display "\n")
                        (not x8113)))))
                   g8110)))
               (string<=?
                (lambda (s1 s2)
                  (letrec*
                   ((g8114
                     (letrec*
                      ((val7531
                        (begin
                          (write '(funapp 1371 33))
                          (display "\n")
                          (string<? s1 s2))))
                      (letrec*
                       ((g8115
                         (if val7531
                           val7531
                           (begin
                             (write '(funapp 1373 52))
                             (display "\n")
                             (string=? s1 s2)))))
                       g8115))))
                   g8114)))
               (for-each
                (lambda (f l)
                  (letrec*
                   ((g8116
                     (letrec*
                      ((x8119
                        (begin
                          (write '(funapp 1379 45))
                          (display "\n")
                          (procedure? f))))
                      (begin
                        (write '(funapp 1379 62))
                        (display "\n")
                        (assert x8119))))
                    (g8117
                     (letrec*
                      ((x8120
                        (begin
                          (write '(funapp 1380 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1380 57))
                        (display "\n")
                        (assert x8120))))
                    (g8118
                     (letrec*
                      ((x-cnd8121
                        (begin
                          (write '(funapp 1383 35))
                          (display "\n")
                          (null? l))))
                      (if x-cnd8121
                        #t
                        (letrec*
                         ((x-cnd8122
                           (begin
                             (write '(funapp 1387 38))
                             (display "\n")
                             (pair? l))))
                         (if x-cnd8122
                           (letrec*
                            ((g8123
                              (letrec*
                               ((x8125
                                 (begin
                                   (write '(funapp 1390 54))
                                   (display "\n")
                                   (car l))))
                               (begin
                                 (write '(funapp 1390 64))
                                 (display "\n")
                                 (f x8125))))
                             (g8124
                              (letrec*
                               ((x8126
                                 (begin
                                   (write '(funapp 1392 48))
                                   (display "\n")
                                   (cdr l))))
                               (begin
                                 (write '(funapp 1392 58))
                                 (display "\n")
                                 (for-each f x8126)))))
                            g8124)
                           (begin
                             (write '(funapp 1394 27))
                             (display "\n")
                             '())))))))
                   g8118)))
               (abs
                (lambda (x)
                  (letrec*
                   ((g8127
                     (letrec*
                      ((x8129
                        (begin
                          (write '(funapp 1399 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 1399 59))
                        (display "\n")
                        (assert x8129))))
                    (g8128
                     (letrec*
                      ((x-cnd8130
                        (begin
                          (write '(funapp 1401 43))
                          (display "\n")
                          (< x 0))))
                      (if x-cnd8130
                        (begin
                          (write '(funapp 1401 67))
                          (display "\n")
                          (- 0 x))
                        x))))
                   g8128)))
               (char-ci>=?
                (lambda (c1 c2)
                  (letrec*
                   ((g8131
                     (letrec*
                      ((x8134
                        (begin
                          (write '(funapp 1406 45))
                          (display "\n")
                          (char? c1))))
                      (begin
                        (write '(funapp 1406 58))
                        (display "\n")
                        (assert x8134))))
                    (g8132
                     (letrec*
                      ((x8135
                        (begin
                          (write '(funapp 1407 45))
                          (display "\n")
                          (char? c2))))
                      (begin
                        (write '(funapp 1407 58))
                        (display "\n")
                        (assert x8135))))
                    (g8133
                     (letrec*
                      ((val7532
                        (begin
                          (write '(funapp 1410 33))
                          (display "\n")
                          (char-ci>? c1 c2))))
                      (letrec*
                       ((g8136
                         (if val7532
                           val7532
                           (begin
                             (write '(funapp 1412 52))
                             (display "\n")
                             (char-ci=? c1 c2)))))
                       g8136))))
                   g8133)))
               (caddar
                (lambda (x)
                  (letrec*
                   ((g8137
                     (letrec*
                      ((x8138
                        (letrec*
                         ((x8139
                           (letrec*
                            ((x8140
                              (begin
                                (write '(funapp 1422 51))
                                (display "\n")
                                (car x))))
                            (begin
                              (write '(funapp 1422 61))
                              (display "\n")
                              (cdr x8140)))))
                         (begin
                           (write '(funapp 1423 26))
                           (display "\n")
                           (cdr x8139)))))
                      (begin
                        (write '(funapp 1424 23))
                        (display "\n")
                        (car x8138)))))
                   g8137)))
               (newline (lambda () (letrec* ((g8141 #f)) g8141)))
               (lcm
                (lambda (m n)
                  (letrec*
                   ((g8142
                     (letrec*
                      ((x8144
                        (letrec*
                         ((x8145
                           (begin
                             (write '(funapp 1432 48))
                             (display "\n")
                             (* m n))))
                         (begin
                           (write '(funapp 1432 58))
                           (display "\n")
                           (abs x8145))))
                       (x8143
                        (begin
                          (write '(funapp 1433 31))
                          (display "\n")
                          (gcd m n))))
                      (begin
                        (write '(funapp 1434 23))
                        (display "\n")
                        (/ x8144 x8143)))))
                   g8142)))
               (deref car)
               (>
                (lambda (x y)
                  (letrec*
                   ((g8146
                     (letrec*
                      ((x8148
                        (begin
                          (write '(funapp 1440 45))
                          (display "\n")
                          (number? x))))
                      (begin
                        (write '(funapp 1440 59))
                        (display "\n")
                        (assert x8148))))
                    (g8147
                     (letrec*
                      ((x8149
                        (begin
                          (write '(funapp 1441 45))
                          (display "\n")
                          (<= x y))))
                      (begin
                        (write '(funapp 1441 56))
                        (display "\n")
                        (not x8149)))))
                   g8147)))
               (list-ref
                (lambda (l index)
                  (letrec*
                   ((g8150
                     (letrec*
                      ((x8154
                        (begin
                          (write '(funapp 1446 45))
                          (display "\n")
                          (list? l))))
                      (begin
                        (write '(funapp 1446 57))
                        (display "\n")
                        (assert x8154))))
                    (g8151
                     (letrec*
                      ((x8155
                        (begin
                          (write '(funapp 1447 45))
                          (display "\n")
                          (number? index))))
                      (begin
                        (write '(funapp 1447 63))
                        (display "\n")
                        (assert x8155))))
                    (g8152
                     (letrec*
                      ((x8156
                        (letrec*
                         ((x8157
                           (begin
                             (write '(funapp 1450 48))
                             (display "\n")
                             (length l))))
                         (begin
                           (write '(funapp 1450 61))
                           (display "\n")
                           (< index x8157)))))
                      (begin
                        (write '(funapp 1451 23))
                        (display "\n")
                        (assert x8156))))
                    (g8153
                     (letrec*
                      ((x-cnd8158
                        (begin
                          (write '(funapp 1454 35))
                          (display "\n")
                          (= index 0))))
                      (if x-cnd8158
                        (begin
                          (write '(funapp 1456 25))
                          (display "\n")
                          (car l))
                        (letrec*
                         ((x8160
                           (begin
                             (write '(funapp 1458 34))
                             (display "\n")
                             (cdr l)))
                          (x8159
                           (begin
                             (write '(funapp 1458 50))
                             (display "\n")
                             (- index 1))))
                         (begin
                           (write '(funapp 1459 26))
                           (display "\n")
                           (list-ref x8160 x8159)))))))
                   g8153)))
               (gcd
                (lambda (a b)
                  (letrec*
                   ((g8161
                     (letrec*
                      ((x-cnd8162
                        (begin
                          (write '(funapp 1466 35))
                          (display "\n")
                          (= b 0))))
                      (if x-cnd8162
                        a
                        (letrec*
                         ((x8163
                           (begin
                             (write '(funapp 1469 42))
                             (display "\n")
                             (modulo a b))))
                         (begin
                           (write '(funapp 1469 57))
                           (display "\n")
                           (gcd b x8163)))))))
                   g8161)))
               (phi
                (letrec*
                 ((x8189 (begin (write '(funapp 1473 26)) (display "\n") (x1)))
                  (x8164
                   (letrec*
                    ((x8188
                      (begin (write '(funapp 1476 29)) (display "\n") (x2)))
                     (x8165
                      (letrec*
                       ((x8187
                         (begin (write '(funapp 1479 32)) (display "\n") (x3)))
                        (x8166
                         (letrec*
                          ((x8186
                            (begin
                              (write '(funapp 1482 35))
                              (display "\n")
                              (x4)))
                           (x8167
                            (letrec*
                             ((x8185
                               (begin
                                 (write '(funapp 1485 38))
                                 (display "\n")
                                 (x5)))
                              (x8168
                               (letrec*
                                ((x8184
                                  (begin
                                    (write '(funapp 1488 41))
                                    (display "\n")
                                    (x6)))
                                 (x8169
                                  (letrec*
                                   ((x8183
                                     (begin
                                       (write '(funapp 1491 44))
                                       (display "\n")
                                       (x7)))
                                    (x8170
                                     (letrec*
                                      ((x-cnd8171
                                        (letrec*
                                         ((val7533 x1))
                                         (letrec*
                                          ((g8172 (if val7533 val7533 x2)))
                                          g8172))))
                                      (if x-cnd8171
                                        (letrec*
                                         ((x-cnd8173
                                           (letrec*
                                            ((val7534 x1))
                                            (letrec*
                                             ((g8174
                                               (if val7534
                                                 val7534
                                                 (letrec*
                                                  ((val7535
                                                    (begin
                                                      (write '(funapp 1510 61))
                                                      (display "\n")
                                                      (not x2))))
                                                  (letrec*
                                                   ((g8175
                                                     (if val7535
                                                       val7535
                                                       (begin
                                                         (write
                                                          '(funapp 1515 56))
                                                         (display "\n")
                                                         (not x3)))))
                                                   g8175)))))
                                             g8174))))
                                         (if x-cnd8173
                                           (letrec*
                                            ((x-cnd8176
                                              (letrec*
                                               ((val7536 x3))
                                               (letrec*
                                                ((g8177
                                                  (if val7536 val7536 x4)))
                                                g8177))))
                                            (if x-cnd8176
                                              (letrec*
                                               ((x-cnd8178
                                                 (letrec*
                                                  ((val7537
                                                    (begin
                                                      (write '(funapp 1531 61))
                                                      (display "\n")
                                                      (not x4))))
                                                  (letrec*
                                                   ((g8179
                                                     (if val7537 val7537 x1)))
                                                   g8179))))
                                               (if x-cnd8178
                                                 (letrec*
                                                  ((x-cnd8180
                                                    (letrec*
                                                     ((val7538
                                                       (begin
                                                         (write
                                                          '(funapp 1540 64))
                                                         (display "\n")
                                                         (not x2))))
                                                     (letrec*
                                                      ((g8181
                                                        (if val7538
                                                          val7538
                                                          (begin
                                                            (write
                                                             '(funapp 1545 59))
                                                            (display "\n")
                                                            (not x3)))))
                                                      g8181))))
                                                  (if x-cnd8180
                                                    (letrec*
                                                     ((val7539 x4))
                                                     (letrec*
                                                      ((g8182
                                                        (if val7539
                                                          val7539
                                                          x2)))
                                                      g8182))
                                                    #f))
                                                 #f))
                                              #f))
                                           #f))
                                        #f))))
                                   (begin
                                     (write '(funapp 1561 36))
                                     (display "\n")
                                     (λ x8183 x8170)))))
                                (begin
                                  (write '(funapp 1562 33))
                                  (display "\n")
                                  (λ x8184 x8169)))))
                             (begin
                               (write '(funapp 1563 30))
                               (display "\n")
                               (λ x8185 x8168)))))
                          (begin
                            (write '(funapp 1564 27))
                            (display "\n")
                            (λ x8186 x8167)))))
                       (begin
                         (write '(funapp 1565 24))
                         (display "\n")
                         (λ x8187 x8166)))))
                    (begin
                      (write '(funapp 1566 21))
                      (display "\n")
                      (λ x8188 x8165)))))
                 (begin
                   (write '(funapp 1567 18))
                   (display "\n")
                   (λ x8189 x8164))))
               (try
                (letrec*
                 ((x8192 (begin (write '(funapp 1570 26)) (display "\n") (f)))
                  (x8190
                   (letrec*
                    ((val7540
                      (begin (write '(funapp 1573 31)) (display "\n") (f #t))))
                    (letrec*
                     ((g8191
                       (if val7540
                         val7540
                         (begin
                           (write '(funapp 1574 58))
                           (display "\n")
                           (f #f)))))
                     g8191))))
                 (begin
                   (write '(funapp 1575 18))
                   (display "\n")
                   (λ x8192 x8190))))
               (sat-solve-7
                (letrec*
                 ((x8221 (begin (write '(funapp 1578 26)) (display "\n") (p)))
                  (x8193
                   (letrec*
                    ((x8194
                      (letrec*
                       ((x8220
                         (begin (write '(funapp 1583 32)) (display "\n") (n1)))
                        (x8195
                         (letrec*
                          ((x8196
                            (letrec*
                             ((x8219
                               (begin
                                 (write '(funapp 1588 38))
                                 (display "\n")
                                 (n2)))
                              (x8197
                               (letrec*
                                ((x8198
                                  (letrec*
                                   ((x8218
                                     (begin
                                       (write '(funapp 1593 44))
                                       (display "\n")
                                       (n3)))
                                    (x8199
                                     (letrec*
                                      ((x8200
                                        (letrec*
                                         ((x8217
                                           (begin
                                             (write '(funapp 1598 50))
                                             (display "\n")
                                             (n4)))
                                          (x8201
                                           (letrec*
                                            ((x8202
                                              (letrec*
                                               ((x8216
                                                 (begin
                                                   (write '(funapp 1603 56))
                                                   (display "\n")
                                                   (n5)))
                                                (x8203
                                                 (letrec*
                                                  ((x8204
                                                    (letrec*
                                                     ((x8215
                                                       (begin
                                                         (write
                                                          '(funapp 1608 62))
                                                         (display "\n")
                                                         (n6)))
                                                      (x8205
                                                       (letrec*
                                                        ((x8206
                                                          (letrec*
                                                           ((x8214
                                                             (begin
                                                               (write
                                                                '(funapp
                                                                  1613
                                                                  68))
                                                               (display "\n")
                                                               (n7)))
                                                            (x8207
                                                             (letrec*
                                                              ((x8208
                                                                (letrec*
                                                                 ((x8209
                                                                   (letrec*
                                                                    ((x8210
                                                                      (letrec*
                                                                       ((x8211
                                                                         (letrec*
                                                                          ((x8212
                                                                            (letrec*
                                                                             ((x8213
                                                                               (begin
                                                                                 (write
                                                                                  '(funapp
                                                                                    1627
                                                                                    80))
                                                                                 (display
                                                                                  "\n")
                                                                                 (p
                                                                                  n1))))
                                                                             (begin
                                                                               (write
                                                                                '(funapp
                                                                                  1629
                                                                                  78))
                                                                               (display
                                                                                "\n")
                                                                               (x8213
                                                                                n2)))))
                                                                          (begin
                                                                            (write
                                                                             '(funapp
                                                                               1631
                                                                               75))
                                                                            (display
                                                                             "\n")
                                                                            (x8212
                                                                             n3)))))
                                                                       (begin
                                                                         (write
                                                                          '(funapp
                                                                            1633
                                                                            72))
                                                                         (display
                                                                          "\n")
                                                                         (x8211
                                                                          n4)))))
                                                                    (begin
                                                                      (write
                                                                       '(funapp
                                                                         1635
                                                                         69))
                                                                      (display
                                                                       "\n")
                                                                      (x8210
                                                                       n5)))))
                                                                 (begin
                                                                   (write
                                                                    '(funapp
                                                                      1637
                                                                      66))
                                                                   (display
                                                                    "\n")
                                                                   (x8209
                                                                    n6)))))
                                                              (begin
                                                                (write
                                                                 '(funapp
                                                                   1638
                                                                   63))
                                                                (display "\n")
                                                                (x8208 n7)))))
                                                           (begin
                                                             (write
                                                              '(funapp
                                                                1639
                                                                60))
                                                             (display "\n")
                                                             (λ x8214
                                                               x8207)))))
                                                        (begin
                                                          (write
                                                           '(funapp 1640 57))
                                                          (display "\n")
                                                          (try x8206)))))
                                                     (begin
                                                       (write
                                                        '(funapp 1641 54))
                                                       (display "\n")
                                                       (λ x8215 x8205)))))
                                                  (begin
                                                    (write '(funapp 1642 51))
                                                    (display "\n")
                                                    (try x8204)))))
                                               (begin
                                                 (write '(funapp 1643 48))
                                                 (display "\n")
                                                 (λ x8216 x8203)))))
                                            (begin
                                              (write '(funapp 1644 45))
                                              (display "\n")
                                              (try x8202)))))
                                         (begin
                                           (write '(funapp 1645 42))
                                           (display "\n")
                                           (λ x8217 x8201)))))
                                      (begin
                                        (write '(funapp 1646 39))
                                        (display "\n")
                                        (try x8200)))))
                                   (begin
                                     (write '(funapp 1647 36))
                                     (display "\n")
                                     (λ x8218 x8199)))))
                                (begin
                                  (write '(funapp 1648 33))
                                  (display "\n")
                                  (try x8198)))))
                             (begin
                               (write '(funapp 1649 30))
                               (display "\n")
                               (λ x8219 x8197)))))
                          (begin
                            (write '(funapp 1650 27))
                            (display "\n")
                            (try x8196)))))
                       (begin
                         (write '(funapp 1651 24))
                         (display "\n")
                         (λ x8220 x8195)))))
                    (begin
                      (write '(funapp 1652 21))
                      (display "\n")
                      (try x8194)))))
                 (begin
                   (write '(funapp 1653 18))
                   (display "\n")
                   (λ x8221 x8193)))))
              (letrec*
               ((g8222
                 (begin
                   (write '(funapp 1654 32))
                   (display "\n")
                   (sat-solve-7 phi))))
               g8222))))
           g7644))))
       g7642)))
    g7641)))
