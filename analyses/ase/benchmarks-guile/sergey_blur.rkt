            ...
            (g7344
             (letrec*
              ((empty (begin (write '(funapp 56 22)) (display "\n") '()))
               ...
               (id
                (letrec*
                 ((x7855 (begin (write '(funapp 1424 37)) (display "\n") (x))))
                 (begin (write '(funapp 1424 43)) (display "\n") (λ x7855 x))))
               (blur
                (letrec*
                 ((x7856 (begin (write '(funapp 1425 39)) (display "\n") (y))))
                 (begin (write '(funapp 1425 45)) (display "\n") (λ x7856 y))))
               (lp
                (letrec*
                 ((x7868 (begin (write '(funapp 1428 26)) (display "\n") (a)))
                  (x7857
                   (letrec*
                    ((x7867
                      (begin (write '(funapp 1431 29)) (display "\n") (n)))
                     (x7858
                      (letrec*
                       ((x-cnd7859
                         (begin
                           (write '(funapp 1434 36))
                           (display "\n")
                           (zero? n))))
                       (if x-cnd7859
                         (begin
                         ...
                         (letrec*
                          ((r
                            (letrec*
                             ((x7860
                               (begin
                                 (write '(funapp 1438 48))
                                 (display "\n")
                                 (blur id))))
                             (begin
                               (write '(funapp 1438 60))
                               (display "\n")
                               (x7860 #t))))
                           (s
                            (letrec*
                             ((x7861
                               (begin
                                 (write '(funapp 1439 48))
                                 (display "\n")
                                 (blur id))))
                             (begin
                               (write '(funapp 1439 60))
                               (display "\n")
                               (x7861 #f)))))
                          (letrec*
                           ((g7862
                                 ...
                                 ((x7865
                                  ...
                                  (x7864
                                   (begin
                                     ...
                                     (sub1 n))))
                                 (begin
                                   (write '(funapp 1448 34))
                                   (display "\n")
                                   (x7865 x7864)))))
                              (begin
                                (write '(funapp 1449 31))
                                (display "\n")
                                (not x7863)))))
                           g7862))))))
                    (begin
                      (write '(funapp 1451 21))
                      (display "\n")
                      (λ x7867 x7858)))))
                 (begin
                   (write '(funapp 1452 18))
                   (display "\n")
                   (λ x7868 x7857)))))
              (letrec*
               ((g7869
                 (letrec*
                  ((x7870
                    (begin (write '(funapp 1454 41)) (display "\n") (lp #f))))
                  (begin (write '(funapp 1454 51)) (display "\n") (x7870 2)))))
               g7869))))
           g7344))))
       g7342)))
    g7341)))
