                                ...
                                (letrec*
                                 ((g7849
                                   (letrec*
                                    ((c
                                      (begin
                                        (write '(funapp 1374 41))
                                        (display "\n")
                                        (f2 #f))))
                                    (letrec*
                                     ((g7850
                                       (begin
                                         (write '(funapp 1375 54))
                                         (display "\n")
                                         (f2 #t))))
                                     g7850))))
                                 g7849))))
                             (begin
                               (write '(funapp 1377 30))
                               (display "\n")
                               (λ x7851 x7848))))
                           (x7839
                            (letrec*
                                ...
                                ((x7843
                                 ...
                                 (x7841
                                  (letrec*
                                   ((x7842
                                     (begin
                                       (write '(funapp 1388 52))
                                       (display "\n")
                                       (y1 y2))))
                                   (begin
                                     (write '(funapp 1388 62))
                                     (display "\n")
                                     (λ x7842 y1)))))
                                (begin
                                  (write '(funapp 1389 33))
                                  (display "\n")
                                  (x7843 x7841)))))
                             (begin
                               (write '(funapp 1390 30))
                               (display "\n")
                               (λ x7846 x7840)))))
                          (begin
                            (write '(funapp 1391 27))
                            (display "\n")
                            (x7847 x7839)))))
                       (begin
                         (write '(funapp 1392 24))
                         (display "\n")
                         (λ x7852 x7838)))))
                    (begin
                      (write '(funapp 1393 21))
                      (display "\n")
                      (x7853 x7837)))))
                 g7836))))
             g7340))))
